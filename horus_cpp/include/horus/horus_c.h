...
/* Each type has, on the publisher: _new, _destroy, _send, _dropped_count; */
/* and on the subscriber: _new, _destroy, _recv, _has_msg, _missed_count.  */
/* The two counters are how a C caller learns a message was lost. A topic  */
/* is lossy by design -- which end drops depends on the backend, the       */
/* broadcast ones overwriting unread slots and the queued ones refusing at */
/* the publisher -- so _recv succeeding says nothing about what it skipped */
/* to get there, and a caller that reads neither counter cannot tell a     */
/* quiet sensor from a dead one.                                           */
/* Sensor */
