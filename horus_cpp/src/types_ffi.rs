...
///
/// `Default` yields an empty-name placeholder node with no rate/budget/deadline,
/// `Warn` miss policy, `BestEffort` class, and `order: 0` (no scheduling
/// preference — the dependency graph handles ordering).
#[derive(Default)]
