//! Python bindings for [`NodeParams`] — typed access to hardware config values.
...
///
/// Wraps the params from a `[hardware.NAME]` config table.  Values are
/// auto-converted from TOML types to Python types (str, int, float, bool, list).
#[pyclass(name = "NodeParams")]
pub struct PyDriverParams {
    pub(crate) inner: NodeParams,
}
...
fn toml_to_python(params: &NodeParams, py: Python<'_>, key: &str) -> PyResult<Py<PyAny>> {
    // Scalars
