...
impl PyJointState {
    ...
    fn positions(&self) -> Vec<f64> {
        self.inner.positions[..(self.inner.joint_count as usize).min(self.inner.positions.len())]
            .to_vec()
    }

    #[getter]
    fn velocities(&self) -> Vec<f64> {
        self.inner.velocities[..(self.inner.joint_count as usize).min(self.inner.velocities.len())]
            .to_vec()
    }

    #[getter]
    fn efforts(&self) -> Vec<f64> {
        self.inner.efforts[..(self.inner.joint_count as usize).min(self.inner.efforts.len())]
            .to_vec()
    }
