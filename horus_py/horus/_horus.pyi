...
class PyHub:
    """
    Python wrapper for HORUS Hub - provides pub/sub communication.

    Hub manages publish/subscribe messaging between nodes using
    shared memory for efficient zero-copy communication.
    """

    def __init__(self, topic: str, capacity: int) -> None:
        """
        Create a new Hub for a topic.
    ...
    def send(self, message: Any) -> bool:
        ...
        The message can be any Python object that can be serialized
        (dict, list, str, int, float, bytes, or pickle-able objects).
    ...
    def send_with_metadata(self, data: bytes, msg_type: str) -> bool:
        """
        Send bytes with type metadata.

        Args:
            data: Bytes to send
            msg_type: Message type ("json", "pickle", etc.)
    ...
    def recv(self) -> Optional[Any]:
        ...
        Returns:
            Received message as bytes, or None if no messages available
        """
        ...

    def recv_with_metadata(self) -> Optional[tuple[Any, str, float]]:
        """
        Try to receive a message with metadata.

        Returns:
            Tuple of (data_bytes, msg_type, timestamp) if message available,
            None otherwise
        """
        ...
