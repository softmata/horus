...
fn every_tutorial_exists_in_every_language() {
    ...
    for entry in std::fs::read_dir(&dir).expect("tutorials/ must be readable").flatten() {
        let path = entry.path();
        if !path.extension().is_some_and(|e| e == "mdx") {
            continue;
        }
...
fn env_names_matching(src: &str, pat: &str, skip: &[(usize, usize)], out: &mut Vec<(String, usize)>) {
...
fn concepts_pages_do_not_restate_the_language_references() {
    ...
    let concepts: Vec<&(String, std::collections::BTreeSet<String>)> =
        pages.iter().filter(|(p, _)| p.starts_with("concepts/")).collect();
