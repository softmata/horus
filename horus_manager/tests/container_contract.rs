...
fn stages() -> (Vec<String>, HashMap<String, Stage>) {
    ...
    for line in src.lines() {
        ...
        if let Some(name) = &current {
            if !t.is_empty() {
                map.get_mut(name).expect("stage exists").body.push(t.to_string());
            }
        }
    }
    (order, map)
...
fn ci_builds_the_root_dockerfile_and_the_devcontainer() {
    ...
    let target = devcontainer_target();
    let builds_devcontainer = wfs.iter().any(|(_, body)| {
        body.contains("devcontainer.json") && body.contains("docker build")
    }) || wfs
        .iter()
