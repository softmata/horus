...
fn build_ffi(project: &Path) -> PathBuf {
    ...
    assert!(status.success(), "cargo build failed in {}", crate_dir.display());
...
fn built() -> Option<&'static Built> {
    ...
    FIXTURE
        .get_or_init(|| {
            ...
            let v1 = tmp.path().join("v1").with_extension(so.extension().unwrap_or_default());
            std::fs::copy(&so, &v1).expect("copy v1");

            std::fs::write(project.join("msgs/m.hmsg"), definitions(POSE_YX)).expect("write");
            regenerate(&project);
            let so = build_ffi(&project);
            let v2 = tmp.path().join("v2").with_extension(so.extension().unwrap_or_default());
...
fn the_ffi_crate_is_built_as_a_loadable_library() {
    let (_tmp, project) = generated_project(&definitions(POSE_XY));
    let manifest =
        std::fs::read_to_string(project.join(".horus/generated/msgs_ffi/Cargo.toml"))
            .expect("manifest");
    let types = manifest
        .lines()
