...
fn no_short_flag_acquires_a_second_meaning() {
    ...
    let mut meanings: std::collections::BTreeMap<char, std::collections::BTreeMap<String, Vec<String>>> =
...
fn env_help_names_every_tool_the_shell_integration_shadows() {
    ...
    let shadowed: BTreeSet<String> = env_rs
        ...
        .filter(|n| !n.is_empty() && n.chars().all(|c| c.is_ascii_lowercase() || c.is_ascii_digit()))
        .map(|n| n.to_string())
        .collect();
