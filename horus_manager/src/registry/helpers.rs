...
pub(crate) fn handle_system_dependencies(deps: &[String]) -> Result<()> {
    ...
    if missing.is_empty() {
        println!(
            "  {} All system packages already installed",
...
fn inject_horus_path_overrides(package_dir: &Path) -> Result<()> {
    ...
    let base = if horus_source.ends_with(".horus/cache") || horus_source.ends_with(".horus\\cache")
    {
    ...
    let mut config = String::from(
        "# Auto-generated by horus pkg install for build-time dep resolution\n[patch.crates-io]\n",
    );
