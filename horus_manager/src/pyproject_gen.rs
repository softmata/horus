...
fn collect_python_deps(deps: &std::collections::BTreeMap<String, DependencyValue>) -> Vec<String> {
...
mod tests {
    ...
    fn battle_pep508_git_with_tag() {
        ...
        deps.insert(
            "my-lib".into(),
            py_git_dep("https://github.com/org/my-lib", None, Some("v2.0.0"), None),
        );
        ...
        assert!(
            content.contains("my-lib @ git+https://github.com/org/my-lib@v2.0.0"),
            "git+tag format wrong: {}",
            content
        );
    }
    ...
    fn battle_pep508_git_with_rev() {
        ...
        deps.insert(
            "torch-custom".into(),
            py_git_dep(
                "https://github.com/org/torch-custom",
                None,
                None,
                Some("abc123"),
            ),
        ...
        assert!(
            content.contains("torch-custom @ git+https://github.com/org/torch-custom@abc123"),
            "git+rev format wrong: {}",
            content
        );
    }
    ...
    fn battle_pep508_path_dep() {
        ...
        deps.insert(
            "local-utils".into(),
            py_path_dep("/home/user/libs/local-utils"),
        );
        ...
        assert!(
            content.contains("local-utils @ file:///home/user/libs/local-utils"),
            "path dep format wrong: {}",
            content
        );
    }
    ...
    fn battle_version_operators() {
        ...
        assert!(
            content.contains("\"scipy<2.0\""),
            "lt version wrong: {}",
            content
        );
        assert!(
            content.contains("\"pandas~=1.5\""),
            "compatible version wrong: {}",
            content
        );
        assert!(
            content.contains("\"pillow!=9.0\""),
            "ne version wrong: {}",
            content
        );
        assert!(
    ...
    fn battle_extras_multiple_features() {
        ...
        assert!(
            content.contains("\"fastapi>=0.100[all,jinja]\""),
            "multiple extras wrong: {}",
            content
        );
    }
    ...
    fn battle_mixed_deps_only_python() {
        ...
        deps.insert(
            "my-py-lib".into(),
            py_git_dep("https://github.com/org/my-py-lib", Some("main"), None, None),
        );
        ...
        assert!(
            !content.contains("horus-nav"),
            "registry dep should be filtered"
        );
        assert!(
    ...
    fn battle_author_propagation_single() {
        ...
        assert!(
            content.contains("name = \"Test Author\""),
            "author name missing: {}",
            content
        );
    }
    ...
    fn battle_no_authors_no_section() {
        ...
        assert!(
            ...
            "empty authors should not produce section"
        );
    }
    ...
    fn battle_dev_deps_only_python_filtered() {
        ...
        assert!(
            ...
            "criterion (crates.io) should be filtered"
        );
    }
    ...
    fn battle_dev_deps_all_non_python_no_section() {
        ...
        assert!(
            !content.contains("[project.optional-dependencies]"),
            "should not have optional-dependencies when all dev deps are Rust: {}",
            content
        );
    }
    ...
    fn battle_build_system_always_present() {
        ...
        assert!(
            content.contains("setuptools>=68.0"),
            "setuptools version missing"
        );
        assert!(
            content.contains("build-backend = \"setuptools.build_meta\""),
    ...
    fn battle_tool_configs_always_present() {
        ...
        assert!(
            content.contains("[tool.setuptools.packages.find]"),
            "setuptools packages config missing"
        );
        assert!(
            content.contains("where = [\"..\"]"),
            "package find config missing"
        );
        assert!(content.contains("[tool.ruff]"), "ruff config missing");
        assert!(
            content.contains("line-length = 100"),
            "ruff line-length missing"
        );
        assert!(
            content.contains("target-version = \"py310\""),
            "ruff target missing"
        );
    }
