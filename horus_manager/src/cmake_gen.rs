...
pub fn generate(
    ...
    // ── Header ───────────────────────────────────────────────────────────
    writeln!(cmake, "# Generated by horus from horus.toml — do not edit manually")?;
...
mod tests {
    ...
    fn include_directories_present() {
        ...
        let result = generate(&manifest, dir.path(), false).unwrap();
        let content = fs::read_to_string(result).unwrap();
        assert!(content.contains("target_include_directories(my_robot PRIVATE ${CMAKE_SOURCE_DIR}/../include)"));
    }
    ...
    fn battle_generate_idempotent() {
        ...
        assert_eq!(content1, content2, "repeated generation should produce identical output");
    }
    ...
    fn battle_generate_with_all_known_deps() {
        ...
        // Add ALL known deps
        for name in &["eigen", "opencv", "pcl", "boost", "gtest", "fmt", "spdlog", "yaml-cpp", "nlohmann-json"] {
            deps.insert(
                name.to_string(),
                cpp_known_dep(name),
            );
    ...
    fn battle_generate_dev_deps_separate() {
        ...
        let result = generate(&manifest, dir.path(), false).unwrap();
        let content = fs::read_to_string(result).unwrap();
        assert!(!content.contains("find_package(GTest"), "gtest should not appear without include_dev");

        // With include_dev — gtest IN find_package
        let result = generate(&manifest, dir.path(), true).unwrap();
        let content = fs::read_to_string(result).unwrap();
        assert!(content.contains("find_package(GTest REQUIRED)"), "gtest should appear with include_dev");
    }
    ...
    fn battle_is_cpp_dep_by_cmake_package() {
        ...
        assert!(is_cpp_dep("mylib", &dep), "cmake_package should make it a C++ dep");
    }

    #[test]
    fn battle_lookup_known_dep_public_all_entries() {
        // Verify every known dep is accessible via public API
        for name in &["eigen", "opencv", "pcl", "boost", "gtest", "fmt", "spdlog", "yaml-cpp", "nlohmann-json", "protobuf", "grpc", "abseil"] {
            assert!(
    ...
    fn battle_apt_package_for_all_known() {
        ...
        assert_eq!(apt_package_for("eigen", &dummy), Some("libeigen3-dev".to_string()));
        assert_eq!(apt_package_for("opencv", &dummy), Some("libopencv-dev".to_string()));
        assert_eq!(apt_package_for("gtest", &dummy), Some("libgtest-dev".to_string()));
        assert_eq!(apt_package_for("fmt", &dummy), Some("libfmt-dev".to_string()));
        assert_eq!(apt_package_for("spdlog", &dummy), Some("libspdlog-dev".to_string()));
        assert_eq!(apt_package_for("nlohmann-json", &dummy), Some("nlohmann-json3-dev".to_string()));
        assert_eq!(apt_package_for("protobuf", &dummy), Some("libprotobuf-dev".to_string()));
    }
    ...
    fn property_sanitize_cmake_name_never_empty_for_nonempty_input() {
        ...
        let inputs = [
            "a", "hello", "my-robot", "robot@v2.0", "____", "123", "a!b#c$d",
            "x", "unicode-\u{00e9}", "---", "...", "a b c",
        ];
        for input in &inputs {
    ...
    fn stress_generate_with_50_deps() {
        ...
        for i in 0..50 {
            ...
            assert!(content.contains(&pkg), "missing find_package for dep {}: {}", i, pkg);
        }
    ...
    fn stress_generate_100_times_same_dir() {
        ...
        for i in 0..100 {
            ...
            let content = fs::read_to_string(&path).unwrap();
            if i > 0 {
                assert_eq!(content, last_content, "generation {} differs from previous", i);
            }
    ...
    fn cmake_configure(project_dir: &std::path::Path) -> std::process::Output {
        ...
        std::process::Command::new("cmake")
            .arg("-S").arg(project_dir.join(".horus"))
            .arg("-B").arg(&build_dir)
    ...
    fn create_cpp_project(dir: &std::path::Path, name: &str, standard: Option<&str>, deps: BTreeMap<String, DependencyValue>) {
        // Create src/main.cpp
    ...
    fn real_cmake_build_minimal_project() {
        ...
        assert!(configure.status.success(), "configure failed: {}", String::from_utf8_lossy(&configure.stderr));
    ...
    fn real_cmake_cpp17_standard() {
        ...
        let output = cmake_configure(dir.path());
        assert!(output.status.success(), "c++17 configure failed: {}", String::from_utf8_lossy(&output.stderr));
    }
    ...
    fn real_cmake_cpp20_standard() {
        ...
        fs::create_dir_all(&src_dir).unwrap();
        fs::write(src_dir.join("main.cpp"), r#"
#include <iostream>
#include <concepts>
#include <ranges>
#include <span>
...
"#).unwrap();
        ...
        let configure = cmake_configure(dir.path());
        assert!(configure.status.success(), "c++20 configure failed: {}", String::from_utf8_lossy(&configure.stderr));
    ...
    fn real_cmake_cpp23_standard() {
        ...
        assert!(output.status.success(), "c++23 configure failed: {}", String::from_utf8_lossy(&output.stderr));
    }
...
"#).unwrap();
        ...
        assert!(stdout.contains("Eigen identity"), "should print identity matrix");
    }
...
"#).unwrap();
        ...
        let configure = cmake_configure(dir.path());
        assert!(configure.status.success(), "cmake should find fmt:\n{}", String::from_utf8_lossy(&configure.stderr));

        let build = cmake_build(dir.path());
        assert!(build.status.success(), "build with fmt should succeed:\n{}", String::from_utf8_lossy(&build.stderr));
    }
...
"#).unwrap();
        ...
        let configure = cmake_configure(dir.path());
        assert!(configure.status.success(), "multi-dep configure failed:\n{}", String::from_utf8_lossy(&configure.stderr));

        let build = cmake_build(dir.path());
        assert!(build.status.success(), "multi-dep build failed:\n{}", String::from_utf8_lossy(&build.stderr));
    }
    ...
    fn real_cmake_gtest_with_tests_dir() {
        ...
        fs::write(test_dir.join("test_basic.cpp"), r#"
...
"#).unwrap();
        ...
        let configure = cmake_configure(dir.path());
        assert!(configure.status.success(), "gtest configure failed:\n{}", String::from_utf8_lossy(&configure.stderr));

        let build = cmake_build(dir.path());
        assert!(build.status.success(), "gtest build failed:\n{}", String::from_utf8_lossy(&build.stderr));

        // Run ctest
        let build_dir = dir.path().join(".horus/cpp-build");
        let ctest = std::process::Command::new("ctest")
            .arg("--test-dir").arg(&build_dir)
    ...
    fn real_cmake_generates_compile_commands() {
        ...
        assert!(content.contains("compdb_test"), "compile_commands.json should reference the project");
        // Verify it's valid JSON
        let parsed: serde_json::Value = serde_json::from_str(&content)
            .expect("compile_commands.json should be valid JSON");
        assert!(parsed.is_array(), "compile_commands.json should be a JSON array");
    }
    ...
    fn real_cmake_hyphenated_project_name() {
        ...
        let configure = cmake_configure(dir.path());
        assert!(configure.status.success(), "hyphenated name should configure:\n{}", String::from_utf8_lossy(&configure.stderr));

        let build = cmake_build(dir.path());
        assert!(build.status.success(), "hyphenated name should build:\n{}", String::from_utf8_lossy(&build.stderr));

        // Binary should be named with underscores
        let binary = dir.path().join(".horus/cpp-build/my_cool_robot");
        assert!(binary.exists(), "binary should be my_cool_robot (underscores), not my-cool-robot");
    }
    ...
    fn real_cmake_compiler_override_gpp() {
        ...
        let configure = cmake_configure(dir.path());
        assert!(configure.status.success(), "g++ override should configure:\n{}", String::from_utf8_lossy(&configure.stderr));

        let build = cmake_build(dir.path());
        assert!(build.status.success(), "g++ override should build:\n{}", String::from_utf8_lossy(&build.stderr));
    }
    ...
    fn real_cmake_custom_args_passthrough() {
        ...
        assert!(content.contains("set(MY_CUSTOM_VAR \"hello\")"), "custom cmake arg should be in CMakeLists.txt");

        let configure = cmake_configure(dir.path());
        assert!(configure.status.success(), "custom cmake args should configure:\n{}", String::from_utf8_lossy(&configure.stderr));
    }
    ...
    fn real_cmake_release_build() {
        ...
        let configure = std::process::Command::new("cmake")
            .arg("-S").arg(dir.path().join(".horus"))
            .arg("-B").arg(&build_dir)
            .arg("-DCMAKE_BUILD_TYPE=Release")
            .stdout(std::process::Stdio::piped())
            .stderr(std::process::Stdio::piped())
            .output().unwrap();
        assert!(configure.status.success(), "Release configure failed:\n{}", String::from_utf8_lossy(&configure.stderr));
    ...
    fn real_cmake_toolchain_file_accepted() {
        ...
        let output = std::process::Command::new("cmake")
            .arg("-S").arg(dir.path().join(".horus"))
            .arg("-B").arg(&build_dir)
            .arg(format!("-DCMAKE_TOOLCHAIN_FILE={}", tc_dir.join("toolchain.cmake").display()))
            .stdout(std::process::Stdio::piped())
            .stderr(std::process::Stdio::piped())
            .output().unwrap();
    ...
    fn real_cmake_incremental_build() {
        ...
        assert!(b2.status.success(), "incremental build failed:\n{}", String::from_utf8_lossy(&b2.stderr));
    }
    ...
    fn real_cmake_non_cpp_deps_excluded() {
        ...
        let content = fs::read_to_string(dir.path().join(".horus/CMakeLists.txt")).unwrap();
        assert!(content.contains("find_package(Eigen3"), "eigen should be in CMakeLists.txt");
        assert!(!content.contains("serde"), "Rust dep should NOT be in CMakeLists.txt");
        assert!(!content.contains("numpy"), "Python dep should NOT be in CMakeLists.txt");
    }
}
