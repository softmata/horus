...
pub fn generate(
    ...
    // ── Binary targets ───────────────────────────────────────────────────
    // The paths already written as `[[bin]]`, so the layout pass below does
    ...
    if source_files.is_empty() {
        ...
        for file in source_files {
            ...
            emitted_bins.push(file.clone());
        }
    }

    // ── Layout targets: lib, src/bin, tests, examples, benches ───────────
    write_layout_targets(&mut cargo, project_dir, &horus_dir, manifest, &emitted_bins);
    ...
    ignore_generated_dir(&horus_dir, BUILD_DIR_IS_GENERATED, BUILD_DIR_KEEPS);
...
const BUILD_DIR_IS_GENERATED: &str =
    "`.horus/` is a build directory, so its contents are ignored — all but\n\
     # deploy.yaml, the fleet inventory `horus deploy` reads out of the checkout.";

/// What in `.horus/` is not build output, and has to stay committable.
///
/// `deploy.yaml` is the fleet inventory: `load_deploy_yaml` in
/// `commands/deploy.rs` reads exactly `.horus/deploy.yaml` to resolve every
/// named target, and `validate_ssh_inputs` gives "which is part of the
/// checkout" as the reason those hosts and identity paths may be trusted into
/// an ssh argv at all. A blanket `*` therefore gitignored the one file in the
/// directory a team must commit, and quietly withdrew the assumption that
/// security control rests on — `git add -A` staged nothing and
/// `git check-ignore -v` blamed `.horus/.gitignore`.
const BUILD_DIR_KEEPS: &[&str] = &["deploy.yaml"];
...
/// The opening of every `.gitignore` written below — how a later run tells one
/// of its own files from one the user wrote.
const GENERATED_GITIGNORE_HEADER: &str = "# Generated by horus.";
...
///
/// `keep` names the entries that are not build output, and each must be a
/// direct child of `dir`. `*` excludes subdirectories as well as files, and git
/// will not descend into an excluded directory to find a negation for something
/// inside it — so `!sub/file` would be accepted and silently do nothing, while
/// `!file` beside the `*` genuinely re-includes it (`git check-ignore -v` then
/// reports the negation rather than the `*`).
///
/// Written when absent, and rewritten when what is there is a file HORUS wrote.
/// A write-only-if-absent rule would have upgraded nobody: every project built
/// before `keep` existed already has a `.gitignore` holding the bare `*` that
/// hid its `deploy.yaml`, and those are precisely the projects with a fleet to
/// deploy to. Anything the user has added to the file is left alone, and so is
/// a directory outside `.horus/` that HORUS did not create — it is their
/// repository. Failure is ignored; this is tidiness, and a build must not stop
/// for it.
fn ignore_generated_dir(dir: &Path, what: &str, keep: &[&str]) {
    let path = dir.join(".gitignore");
    let mut wanted = format!("{GENERATED_GITIGNORE_HEADER} {what}\n*\n");
    for name in keep {
        writeln!(wanted, "!{name}").unwrap();
    }

    let existing = fs::read_to_string(&path).ok();
    let ours_to_write = match existing.as_deref() {
        Some(text) => horus_wrote_gitignore(text, keep),
        // Absent is ours to write. A file that is there but unreadable is not
        // something to claim, so it is not enough that the read failed.
        None => !path.exists(),
    };
    // Rewriting identical bytes on every build would touch the mtime of a file
    // in the user's tree for nothing.
    if !ours_to_write || existing.as_deref() == Some(wanted.as_str()) {
        return;
    }
    let _ = fs::write(&path, wanted);
}

/// Whether a `.gitignore` already in place is one HORUS wrote.
///
/// The test `generated_by_horus` applies to the cargo config — the header HORUS
/// stamps on the file — plus the body, because the header alone would let an
/// upgrade overwrite a rule the user had added underneath it. Every line that
/// is neither a comment nor blank has to be a pattern `ignore_generated_dir`
/// itself emits: the `*`, or a negation for one of `keep`. That accepts the
/// bare `*` older versions wrote, which is the file that needs upgrading, and
/// accepts this version's own output, so a rewrite is idempotent.
fn horus_wrote_gitignore(text: &str, keep: &[&str]) -> bool {
    text.starts_with(GENERATED_GITIGNORE_HEADER)
        && text
            .lines()
...
fn write_bin_entry(cargo: &mut String, name: &str, path: &str) {
    write_target_entry(cargo, "bin", name, path);
}

/// One `[[<kind>]]` table (`bin`, `test`, `example`, `bench`).
fn write_target_entry(cargo: &mut String, kind: &str, name: &str, path: &str) {
    writeln!(cargo, "[[{kind}]]").unwrap();
    writeln!(cargo, "name = \"{}\"", sanitize_cargo_name(name)).unwrap();
    writeln!(cargo, "path = \"{}\"", toml_path(path)).unwrap();
    writeln!(cargo).unwrap();
}

/// Emit the target sections cargo would discover if the manifest lived at the
/// project root: `[lib]`, `src/bin/*`, `tests/*`, `examples/*`, `benches/*`.
///
/// WHY THIS EXISTS. `.horus/Cargo.toml` is the only manifest cargo reads in a
/// managed project, and cargo discovers targets relative to the *manifest's*
/// directory — `.horus/`, not the project root. Only `[[bin]]` entries were
/// emitted, so a project's `tests/` directory compiled and ran **zero tests**:
/// `horus test` and `cargo test --manifest-path .horus/Cargo.toml` both
/// reported success with nothing executed, which is worse than failing.
/// Examples and benches had the same hole, and a `--lib` project's
/// `src/lib.rs` was emitted as a `[[bin]]`, so cargo failed looking for a
/// `main` in it.
///
/// The discovery rules match cargo's own: top-level `*.rs` files are targets,
/// `dir/name/main.rs` is a target named `name`, and anything deeper
/// (`tests/common/mod.rs`) is a helper module, not a target.
fn write_layout_targets(
    cargo: &mut String,
    project_dir: &Path,
    horus_dir: &Path,
    manifest: &HorusManifest,
    ...
    if matches!(target, TargetType::Lib | TargetType::Both) {
        ...
        if lib.exists() {
            ...
            writeln!(
                cargo,
                "path = \"{}\"",
                toml_path(relative_to_horus(&lib, horus_dir))
            )
            .unwrap();
            writeln!(cargo).unwrap();
        }
    ...
    for (dir, kind) in [
        (project_dir.join("tests"), "test"),
        (project_dir.join("examples"), "example"),
        (project_dir.join("benches"), "bench"),
    ] {
...
/// The top-level target files in `dir` as `(target name, path)`, sorted by
/// name so the generated manifest is deterministic — the fingerprint compares
/// it byte for byte and a reshuffled manifest would trigger a rebuild.
///
/// `dir/name/main.rs` is one target named `name`; `dir/common/mod.rs` is not a
/// target at all. Both match cargo's auto-discovery rules.
fn target_files(dir: &Path) -> Vec<(String, PathBuf)> {
    ...
    for entry in entries.flatten() {
        ...
        if path.is_file() {
            ...
            if let Some(name) = path.file_name() {
                files.push((name.to_string_lossy().to_string(), path.join("main.rs")));
            }
        }
    }
    files.sort();
...
mod tests {
    ...
    /// Cargo discovers targets relative to the manifest's directory, and the
    /// only manifest it reads is `.horus/Cargo.toml`. A project's `tests/`,
    /// `examples/` and `benches/` therefore have to be declared explicitly or
    /// they compile to nothing: `horus test` used to report success having run
    /// zero tests.
    #[test]
    fn generate_emits_layout_targets() {
        let dir = tempfile::tempdir().unwrap();
        let root = dir.path();
        ...
        fs::write(root.join("tests/smoke.rs"), "#[test] fn t() {}").unwrap();
        fs::write(root.join("tests/common/mod.rs"), "pub fn helper() {}").unwrap();
        fs::write(root.join("examples/demo.rs"), "fn main() {}").unwrap();
        fs::write(root.join("examples/nested/main.rs"), "fn main() {}").unwrap();
        fs::write(root.join("benches/bench.rs"), "fn main() {}").unwrap();
        ...
        // `tests/common/mod.rs` is a helper module, not a target — cargo does
        // not auto-discover it either, and declaring it would compile it as an
        // empty test binary.
        assert!(
            !content.contains("common"),
            "a nested module became a target:\n{content}"
        );
    ...
    /// `--lib` projects must get a `[lib]`, not a `[[bin]]` pointing at
    /// `src/lib.rs` — cargo reads the latter as a binary and fails looking for
    /// a `main` in it.
    #[test]
    fn generate_emits_a_lib_target_for_lib_projects() {
        let dir = tempfile::tempdir().unwrap();
        let root = dir.path();
        fs::create_dir_all(root.join(".horus")).unwrap();
        fs::create_dir_all(root.join("src")).unwrap();
        fs::write(root.join("src/lib.rs"), "pub fn answer() -> u32 { 42 }").unwrap();
    ...
    /// order must not depend on the filesystem's `readdir` order.
    #[test]
    fn layout_targets_are_deterministic() {
        let dir = tempfile::tempdir().unwrap();
        let root = dir.path();
        fs::create_dir_all(root.join(".horus")).unwrap();
        fs::create_dir_all(root.join("src")).unwrap();
        ...
        for name in ["z_last", "a_first", "m_middle"] {
            fs::write(root.join(format!("tests/{name}.rs")), "#[test] fn t() {}").unwrap();
        }

        let manifest = test_manifest(BTreeMap::new());
        let (_, first) = generate(&manifest, root, &[], false).unwrap();
        let (_, second) = generate(&manifest, root, &[], false).unwrap();
        assert_eq!(first, second, "two generations disagreed");

        let a = first.find("a_first").expect("a_first missing");
        let m = first.find("m_middle").expect("m_middle missing");
        let z = first.find("z_last").expect("z_last missing");
        assert!(a < m && m < z, "targets are not name-sorted:\n{first}");
    }
    ...
    /// Almost everything HORUS puts in `.horus/` is a build artifact, and the
    /// project `.gitignore` names those one path at a time — so each new
    /// generated file starts out untracked and visible in `git status`. The
    /// negation has to come after the `*`: gitignore is last-match-wins, so a
    /// `!deploy.yaml` above it would be overridden by the very rule it exists
    /// to carve out of.
    #[test]
    fn the_build_directory_ignores_itself_but_keeps_the_fleet_inventory() {
        let dir = tempfile::tempdir().unwrap();
        let horus_dir = dir.path().join(".horus");
        fs::create_dir_all(&horus_dir).unwrap();

        ignore_generated_dir(&horus_dir, BUILD_DIR_IS_GENERATED, BUILD_DIR_KEEPS);
        let text = fs::read_to_string(horus_dir.join(".gitignore")).unwrap();
        let patterns: Vec<&str> = text.lines().filter(|l| !l.starts_with('#')).collect();
        assert_eq!(patterns, vec!["*", "!deploy.yaml"], "{text}");
    }

    /// The negation only works because `deploy.yaml` sits directly in the
    /// directory the `.gitignore` is in — git does not descend into a directory
    /// `*` excluded, so the same rule spelled for a nested path would be
    /// accepted and do nothing. Only git can answer that, so ask it.
    #[test]
    fn git_agrees_the_fleet_inventory_is_committable() {
        let Some(dir) = scratch_git_repo() else {
            return;
        };
        let horus_dir = dir.path().join(".horus");
        fs::create_dir_all(horus_dir.join("target/debug")).unwrap();
        fs::write(horus_dir.join("deploy.yaml"), "targets:\n").unwrap();
        fs::write(horus_dir.join("fingerprints.json"), "{}\n").unwrap();
        fs::write(horus_dir.join("target/debug/app"), "").unwrap();

        ignore_generated_dir(&horus_dir, BUILD_DIR_IS_GENERATED, BUILD_DIR_KEEPS);

        // check-ignore exits 1 when it ignores nothing, which is the answer
        // wanted for deploy.yaml and refused for everything else.
        assert!(
            !git_ignores(dir.path(), ".horus/deploy.yaml"),
            "the fleet inventory `horus deploy` reads must be committable"
        );
        for build_output in [".horus/fingerprints.json", ".horus/target/debug/app"] {
            assert!(
                git_ignores(dir.path(), build_output),
                "{build_output} is build output and must stay ignored"
            );
        }
    }

    /// A write-only-when-absent fix would reach no existing project: every one
    /// built before `keep` existed already has the bare `*` on disk, and those
    /// are the projects with a fleet. This is that exact file, byte for byte.
    #[test]
    fn a_gitignore_an_older_horus_wrote_is_upgraded() {
        let dir = tempfile::tempdir().unwrap();
        let horus_dir = dir.path().join(".horus");
        fs::create_dir_all(&horus_dir).unwrap();
        fs::write(
            horus_dir.join(".gitignore"),
            "# Generated by horus. `.horus/` is a build directory — nothing in \
             it belongs in version control.\n*\n",
        )
        .unwrap();

        ignore_generated_dir(&horus_dir, BUILD_DIR_IS_GENERATED, BUILD_DIR_KEEPS);
        let upgraded = fs::read_to_string(horus_dir.join(".gitignore")).unwrap();
        assert!(upgraded.contains("\n!deploy.yaml\n"), "{upgraded}");

        // And the second run is a no-op, not a rewrite of its own output.
        ignore_generated_dir(&horus_dir, BUILD_DIR_IS_GENERATED, BUILD_DIR_KEEPS);
        assert_eq!(
    ...
    /// It is the user's repository: a file they have changed is theirs. Both
    /// halves matter — one they wrote outright, and one of ours they added a
    /// rule to, which the header alone would have called ours to overwrite.
    #[test]
    fn an_edited_build_directory_gitignore_is_left_alone() {
        for existing in [
            "!keep-me\n",
            "# Generated by horus. whatever it said.\n*\n!keep-me\n",
        ] {
            let dir = tempfile::tempdir().unwrap();
            let horus_dir = dir.path().join(".horus");
            fs::create_dir_all(&horus_dir).unwrap();
            fs::write(horus_dir.join(".gitignore"), existing).unwrap();
    ...
    /// A git repository to ask about ignore rules, or `None` where there is no
    /// git to ask — the same skip `scratch_project` makes for a missing source
    /// tree.
    fn scratch_git_repo() -> Option<tempfile::TempDir> {
        let dir = tempfile::tempdir().unwrap();
    ...
    /// Whether git ignores `rel` in the repository at `root`.
