...
/// Collect crate names that are auto-generated from the `[drivers]` section.
fn collect_driver_crates(manifest: &HorusManifest) -> Vec<String> {
    use horus_core::drivers::terra_map;
    let mut crates = Vec::new();
    for (_, driver_val) in &manifest.drivers {
        // Config variant with a terra key
        if let crate::manifest::DriverValue::Config(cfg) = driver_val {
            if let Some(terra_name) = &cfg.terra {
                if let Some(info) = terra_map::resolve(terra_name) {
                    crates.push(info.crate_name.to_string());
                }
            }
        }
    }
