...
fn test_discover_shared_memory_with_real_topic() {
    // Create a real topic via horus_core so it gets a valid SHM header with our PID.
    // Bare file writes won't work because find_topics() filters to is_alive processes.
    use horus_core::communication::Topic;

    // Use a unique name to avoid collisions with parallel tests
    let test_topic = format!("_test.discover.{}", std::process::id());
    let topic: Result<Topic<u64>, _> = Topic::new(&test_topic);
    if topic.is_err() {
        // SHM not available in this environment — skip
        return;
    }
    ...
    // If result is Err, that's OK — SHM discovery may not work in all environments
    // Topic cleanup happens automatically when `topic` is dropped (SHM unlink)
}
