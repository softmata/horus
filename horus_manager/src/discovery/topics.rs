...
pub(super) fn discover_shared_memory_uncached() -> HorusResult<Vec<SharedMemoryInfo>> {
    ...
    // horus_sys deduplicates internally (SHM files and .meta files use the
    // same topic name, HashMap merges them).
    let sys_topics = horus_sys::discover::find_topics();
    let topics_dir = shm_topics_dir();

    // Auto-cleanup stale SHM from dead processes (SIGKILL leaves orphans).
    // Delete topic files + meta files for topics whose creator PID is dead.
    for t in &sys_topics {
    ...
    let mut topics: Vec<SharedMemoryInfo> = sys_topics
        .into_iter()
        .filter(|t| t.is_alive) // Filter out dead topics — match ROS2 behavior
        .map(|t| {
            let procs: Vec<u32> = if t.creator_pid > 0 {
                vec![t.creator_pid]
            } else {
                vec![]
            };
            ...
            SharedMemoryInfo {
                ...
                message_rate_hz: 0.0, // Computed below via batch sampling
                status,
                age_string,
    ...
    // Single wait for ALL topics — O(1) time, not O(N).
    {
        ...
        let totals_before: Vec<u64> = topics
            .iter()
            .map(|t| {
                let path = topics_dir.join(&t.topic_name);
                read_topic_messages_total(&path).unwrap_or(0)
            })
            .collect();

        let sample_ms = 500u64;
        std::thread::sleep(std::time::Duration::from_millis(sample_ms));

        for (i, t) in topics.iter_mut().enumerate() {
            let path = topics_dir.join(&t.topic_name);
            let total_after = read_topic_messages_total(&path).unwrap_or(0);
            let delta = total_after.saturating_sub(totals_before[i]);
            t.message_rate_hz = (delta as f32) * (1000.0 / sample_ms as f32);
        }
    }
