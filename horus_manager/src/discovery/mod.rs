...
pub struct NodeStatus {
    ...
    /// Live watchdog severity from registry (0=Ok, 1=Warning, 2=Expired, 3=Critical).
    pub live_watchdog_severity: Option<u8>,
    /// Live P99 tick duration (ns) from registry.
    pub live_p99_tick_ns: Option<u64>,
    /// Whether this node is from a remote machine (via horus_net).
    pub is_remote: bool,
    /// Remote host identifier (empty if local).
    pub host_id: String,
    /// Whether this node is from an external protocol bridge (zenoh, mqtt, etc.).
    pub is_bridged: bool,
    /// Bridge protocol name (empty if not bridged).
    pub bridge_protocol: String,
    /// Launch session this node belongs to (if spawned via `horus launch`).
    pub launch_session: Option<String>,
    /// Name in the launch YAML (may differ from scheduler node name).
