...
const RT_KERNELS: &[RtKernel] = &[
    ...
    RtKernel {
        ...
        install: &["sudo", "dnf", "install", "-y", "kernel-rt", "kernel-rt-core"],
...
fn install_rt_kernel(distro: &str, arch: &str, run: Runner) -> anyhow::Result<()> {
    ...
    println!("  {} Installing {} ...", "→".cyan(), packages.white().bold());
...
fn ensure_memlock_limits_at(
    ...
    if std::path::Path::new(path).exists() {
        println!("  {} Memory limits already configured ({})", "✓".green(), path);
        return Ok(());
    }
...
fn undo_report(
    ...
    match rt_kernel(distro, arch) {
        ...
        None => {
            lines.push("    To remove, uninstall whichever RT kernel this machine has:".to_string());
...
mod rt_kernel_tests {
    ...
    fn a_failed_install_is_never_reported_as_success() {
        ...
        for (distro, result) in results {
            ...
            assert!(err.to_string().contains("Failed to install"), "{distro}: {err}");
        }
    }
    ...
    fn the_limits_file_is_scoped_to_the_invoking_user() {
        ...
        assert!(calls[0].1.contains("alice - memlock unlimited"), "{}", calls[0].1);
    ...
    fn undo_names_the_package_this_distro_actually_installed() {
        ...
        for (distro, want) in expected {
            ...
            let run = rec.runner();
            let text =
                undo_report(missing.to_str().unwrap(), distro, "x86_64", &run).unwrap().join("\n");
            assert!(text.contains(want), "{distro} undo should say `{want}`:\n{text}");
            assert!(
