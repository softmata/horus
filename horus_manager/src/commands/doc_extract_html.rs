...
fn write_messages_section(html: &mut String, doc: &ProjectDoc) {
    ...
    for msg in &messages {
        ...
        if let Some(ref d) = msg.doc {
            let _ = writeln!(html, "<p>{}</p>", escape_html(d));
        }
        html.push_str("<table>\n");
    ...
    for node in &nodes {
        ...
        if let Some(ref details) = node.details {
            if let Some(ref rate) = details.tick_rate {
                let _ = writeln!(html, "<span class=\"badge\">{rate}</span>");
            }
...
fn write_api_reference(html: &mut String, doc: &ProjectDoc) {
    ...
    for module in &doc.modules {
        ...
        for sym in &module.symbols {
            ...
            let _ = writeln!(html, "<div class=\"symbol{dep_class}\">");
            ...
            match sym {
                SymbolDoc::Function(f) => {
                    ...
                    if let Some(ref d) = f.doc {
                        let _ = writeln!(html, "<p class=\"doc\">{}</p>", escape_html(d));
                    }
                }
                SymbolDoc::Struct(s) => {
                    let _ = writeln!(html, "<strong>struct {}</strong>", escape_html(&s.name));
                    if !s.trait_impls.is_empty() {
                        let impls = s.trait_impls.join(", ");
                        let _ = writeln!(html, "<span class=\"impls\">impl {impls}</span>");
                    }
                ...
                SymbolDoc::Trait(t) => {
                    let _ = writeln!(html, "<strong>trait {}</strong>", escape_html(&t.name));
                }
                ...
                _ => {
                    let _ = writeln!(html, "<strong>{}</strong>", escape_html(sym.name()));
                }
            }
...
fn write_coverage_table(html: &mut String, doc: &ProjectDoc) {
    ...
    for module in &doc.modules {
        ...
        let _ = writeln!(html, "<tr><td>{}</td><td>{}/{}</td><td><span style=\"color:{color}\">{:.0}%</span></td></tr>",
            escape_html(&module.path.display().to_string()), documented, total, pct);
    }

    let _ = writeln!(html, "<tr class=\"total\"><td><strong>Total</strong></td><td><strong>{}/{}</strong></td><td><strong>{:.0}%</strong></td></tr>",
