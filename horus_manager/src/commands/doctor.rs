        ...
        Some(manifest) => match manifest.validate() {
            Ok(warnings) => {
                let mut details = vec![
                    ...
                    // Which manifest owns the build. This was implicit: a root
                    // Cargo.toml silently switches `horus build` to
                    // cargo-native and makes `[rust]` inert, and nothing said
                    // so until a setting quietly stopped applying.
                    build_mode_line(&ctx.root),
                ];
...
/// One line naming the build mode, for `check_manifest`'s details.
///
/// The mode is derived, not stored: a root `Cargo.toml` means cargo owns the
/// build (`horus build` runs cargo on it, `[rust]` is inert), otherwise HORUS
/// generates `.horus/Cargo.toml`. `horus eject` switches between them.
fn build_mode_line(root: &std::path::Path) -> String {
    if root.join("Cargo.toml").exists() {
        "build: cargo-native — root Cargo.toml; [rust] in horus.toml is ignored".to_string()
    } else if root.join(".horus/Cargo.toml").exists() {
        "build: managed — .horus/Cargo.toml generated from horus.toml (plain cargo \
         cannot find it; horus eject writes a root Cargo.toml)"
            .to_string()
    } else {
        "build: managed — .horus/Cargo.toml is generated by the first horus build".to_string()
    }
}
