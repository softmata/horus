...
/// Show current lifecycle state of a node (`horus node lifecycle <name>`)
pub fn node_lifecycle(name: &str) -> HorusResult<()> {
    use horus_core::communication::Topic;
    use horus_core::core::lifecycle::LifecycleTransitionResult;

    let state_topic_name = format!("_horus/lifecycle/{}/state", name);
    let state_topic: Topic<LifecycleTransitionResult> =
        Topic::new(&state_topic_name).map_err(|e| {
            HorusError::Config(format!(
                "Node '{}' has no lifecycle topic: {}\n  Is this node implementing LifecycleNode?",
                name, e
            ))
        })?;

    println!("{}", "Lifecycle State".green().bold());
    println!();
    println!("  {} {}", "Node:".cyan(), name.white().bold());

    // Read latest known state if available
    if let Some(result) = state_topic.recv() {
        let state_str = format!("{}", result.new_state);
        let state_colored = match state_str.as_str() {
            "active" => state_str.green(),
            "inactive" => state_str.yellow(),
            "unconfigured" => state_str.dimmed(),
            "shutting_down" | "finalized" => state_str.red(),
            s if s.starts_with("error_processing") => s.red(),
            _ => state_str.normal(),
        };
        println!("  {} {}", "State:".cyan(), state_colored);
        println!("  {} {}", "Last transition:".cyan(), result.command.dimmed());
        if let Some(err) = &result.error {
            println!("  {} {}", "Error:".red(), err);
        }
    } else {
        println!(
            "  {} (no state published yet — node may still be initializing)",
            "State:".cyan()
        );
    }
    ...
    println!();
    println!(
        "  {} horus node configure/activate/deactivate/cleanup {}",
        "Transitions:".dimmed(),
        name
    );
...
/// Send a lifecycle command to a node
fn send_lifecycle_command(name: &str, cmd_str: &str) -> HorusResult<()> {
    use horus_core::communication::Topic;
    use horus_core::core::lifecycle::LifecycleCommand;

    let cmd = match cmd_str {
        "configure" => LifecycleCommand::Configure,
        "activate" => LifecycleCommand::Activate,
        "deactivate" => LifecycleCommand::Deactivate,
        "cleanup" => LifecycleCommand::Cleanup,
        "shutdown" => LifecycleCommand::Shutdown,
        _ => {
            return Err(HorusError::Config(format!(
    ...
    let cmd_topic_name = format!("_horus/lifecycle/{}/cmd", name);
    let cmd_topic: Topic<LifecycleCommand> =
        Topic::new(&cmd_topic_name).map_err(|e| {
            HorusError::Config(format!(
                "Node '{}' has no lifecycle command topic: {}\n  Is this node implementing LifecycleNode?",
                name, e
            ))
        })?;
    ...
    println!(
        "{} Sent '{}' command to node '{}'",
        cli_output::ICON_SUCCESS.green(),
        cmd_str,
        name.white().bold()
    );
    println!(
        "  {} Use 'horus node lifecycle {}' to check the new state",
        "Tip:".dimmed(),
        name
    );
...
/// Trigger `configure` lifecycle transition (`horus node configure <name>`)
pub fn lifecycle_configure(name: &str) -> HorusResult<()> {
    send_lifecycle_command(name, "configure")
}

/// Trigger `activate` lifecycle transition (`horus node activate <name>`)
pub fn lifecycle_activate(name: &str) -> HorusResult<()> {
    send_lifecycle_command(name, "activate")
}

/// Trigger `deactivate` lifecycle transition (`horus node deactivate <name>`)
pub fn lifecycle_deactivate(name: &str) -> HorusResult<()> {
    send_lifecycle_command(name, "deactivate")
}

/// Trigger `cleanup` lifecycle transition (`horus node cleanup <name>`)
pub fn lifecycle_cleanup(name: &str) -> HorusResult<()> {
    send_lifecycle_command(name, "cleanup")
}

/// Trigger `shutdown` lifecycle transition (`horus node shutdown <name>`)
pub fn lifecycle_shutdown(name: &str) -> HorusResult<()> {
    send_lifecycle_command(name, "shutdown")
}
