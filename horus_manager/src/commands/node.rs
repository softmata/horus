...
pub fn list_nodes(verbose: bool, json: bool, category: Option<String>) -> HorusResult<()> {
        ...
        let items: Vec<_> = filtered_nodes
            ...
            .map(|n| {
                serde_json::json!({
                    ...
                    "host_id": n.host_id,
                    "is_bridged": n.is_bridged,
                    "bridge_protocol": n.bridge_protocol,
                    "launch_session": n.launch_session,
                    "launch_name": n.launch_name,
                    "budget_misses": n.live_budget_misses,
                    "deadline_misses": n.live_deadline_misses
                })
            })
            .collect();
    ...
    if verbose {
        ...
        // Compact table view — group by launch session
        let has_sessions = filtered_nodes.iter().any(|n| n.launch_session.is_some());
        let (launched, standalone): (Vec<_>, Vec<_>) = if has_sessions {
            filtered_nodes
                .iter()
                .partition(|n| n.launch_session.is_some())
        } else {
            (Vec::new(), filtered_nodes.iter().collect())
        };
        ...
        let print_node_row = |node: &crate::discovery::NodeStatus| {
            let status = match node.status.as_str() {
                "Running" | "running" => "Running".green(),
                "Idle" | "idle" => "Idle".yellow(),
                "Unreachable" => "Unreachable".yellow(),
                _ => node.status.as_str().red(),
            };
            let source = if node.is_bridged {
        ...
        // Print launched nodes grouped by session
        if !launched.is_empty() {
            let mut sessions: std::collections::BTreeMap<&str, Vec<_>> =
                std::collections::BTreeMap::new();
            for node in &launched {
                let session = node.launch_session.as_deref().unwrap_or("unknown");
                sessions.entry(session).or_default().push(*node);
            }
            for (session, nodes) in &sessions {
                println!(
                    "  {} {} ({} node{})",
                    "Launch:".cyan(),
                    session.white().bold(),
...
pub fn node_info(name: &str) -> HorusResult<()> {
    ...
    if let Some(ws) = node.live_watchdog_severity {
        if ws > 0 {
            let severity = match ws {
                1 => "Warning".yellow(),
                2 => "Expired".red(),
                3 => "Critical".red().bold(),
                _ => "Unknown".dimmed(),
            };
            println!("    {} {}", "Watchdog:".dimmed(), severity);
        }
    }
...
pub fn kill_node(name: &str, force: bool) -> HorusResult<()> {
    ...
    // If this node belongs to a launch session, route through the launch
    // control topic so the launch monitor can track the stop, update the
    // session manifest, and potentially restart the process.
    if let Some(ref session) = node.launch_session {
        ...
        if let Ok(topic) = horus_core::Topic::<super::launch::LaunchControlCommand>::new(&ctl_topic)
        {
            ...
            topic.send(super::launch::LaunchControlCommand::StopNode {
                name: launch_name.to_string(),
            });
            println!(
                "{} StopNode sent to {}",
        ...
        // Fall through to direct Scheduler control if launch topic unavailable
        println!(
            "  {} Launch control topic unavailable, using direct Scheduler control",
            "⚠".yellow()
        );
    }
