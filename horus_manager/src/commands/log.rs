...
pub fn view_logs(
    ...
    println!("{}", "  (showing local + remote logs)".dimmed());
    println!();
    ...
    let mut all = if use_error_buffer {
        GLOBAL_ERROR_BUFFER.get_all()
    } else {
        GLOBAL_LOG_BUFFER.get_all()
    };

    // Merge remote logs (from GLOBAL_REMOTE_LOG_BUFFER)
    // Remote entries have node_name prefixed with host_id (e.g., "a1b2/controller")
    let remote_entries = GLOBAL_REMOTE_LOG_BUFFER.get_all();
    all.extend(remote_entries);
    // Sort by timestamp for interleaved display (best-effort — remote timestamps may differ)
