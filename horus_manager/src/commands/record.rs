...
/// Delete all recording sessions, or those older than a given age.
pub fn run_clean(max_age_days: Option<u64>, dry_run: bool, force: bool) -> HorusResult<()> {
    let manager = RecordingManager::new();
    let sessions = manager.list_sessions().unwrap_or_default();
    ...
    let to_delete: Vec<&String> = if let Some(days) = max_age_days {
        let max_age = std::time::Duration::from_secs(days * 86400);
        sessions
            .iter()
            .filter(|s| {
                // Extract timestamp from session name: recording_{timestamp}_p{pid}
                ...
                false // Can't determine age — keep it
            })
            .collect()
    } else {
        sessions.iter().collect() // Delete all
    };

    if to_delete.is_empty() {
        println!("No sessions match the criteria.");
        return Ok(());
    }

    println!(
        "{} {} of {} session(s) will be deleted{}",
        if dry_run { "⚠" } else { "🗑" },
        to_delete.len(),
        sessions.len(),
    ...
    if dry_run {
        for s in &to_delete {
            println!("  would delete: {}", s);
        }
    } else {
        let mut deleted = 0;
        for s in &to_delete {
            if manager.delete_session(s).is_ok() {
                deleted += 1;
            }
        }
        println!(
