...
/// Run the WebSocket signaling server for P2P connection coordination
#[cfg(feature = "signaling-server")]
pub fn run_signal_server(
    bind: &str,
    port: u16,
    ...
    println!("{}", "HORUS Signaling Server".green().bold());
    println!();
    ...
    // Print startup info
    println!("  {} {}", "Address:".cyan(), bind_addr);
    println!("  {} {}", "Max peers:".cyan(), max_peers);
    if secret.is_some() {
        println!("  {} {}", "Auth:".cyan(), "Shared secret (enabled)".green());
    } else {
        println!("  {} {}", "Auth:".cyan(), "None (open)".yellow());
    }
    println!();
    ...
    rt.block_on(async {
        ...
        println!("{} Press Ctrl+C to stop", "Starting...".green());
        println!();
