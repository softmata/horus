...
impl TensorHandle {
    ...
    /// Get raw CPU pointer to tensor data.
    ///
    /// Returns a null pointer if the tensor is on a device-only backend
    /// (e.g., `CudaDeviceBackend`) where CPU access is not available.
    /// For unified memory (Jetson), returns a valid CPU-accessible pointer.
    #[inline]
    pub fn data_ptr(&self) -> *mut u8 {
        self.pool.data_ptr(&self.tensor)
    }

    /// Get the device (GPU) pointer for this tensor.
    ///
    /// Returns `None` for CPU-only tensors. Returns `Some(ptr)` for
    /// GPU-resident tensors where the pointer can be passed to CUDA kernels.
    /// For unified memory (Jetson), this is the same address as `data_ptr()`.
    ///
    /// # Status
    ///
    /// Currently always returns `None` (GPU backends not yet implemented).
    /// Will return real device pointers once CUDA backends are added in R2.
    #[inline]
    pub fn device_ptr(&self) -> Option<*mut u8> {
        // For MmapBackend (CPU), there is no device pointer.
        // For future GPU backends, this will query the backend's allocation.
        // Currently: infer from tensor device metadata.
        if self.tensor.is_cuda() {
            // GPU tensor — data_ptr may be the device pointer on unified memory
            let ptr = self.pool.data_ptr(&self.tensor);
            if !ptr.is_null() {
                Some(ptr)
            } else {
                None
