...
use crate::memory::backend::{BackendAllocation, MmapBackend, PoolBackend};
...
struct SlotHeader {
    ...
    ///
    /// Upgraded from `AtomicU32` (POOL_VERSION 2) to `AtomicU64` (POOL_VERSION 3).
    /// Prevents refcount overflow under sustained alloc/dealloc cycles.
    /// At u64::MAX ≈ 1.8×10¹⁹ operations, wraparound is impossible.
    refcount: AtomicU64,
...
pub struct TensorPool {
    ...
    /// Backend for data region allocation.
    /// For mmap pools, this is MmapBackend pointing into the data region
    /// of the same mmap. For GPU backends, this is a CUDA allocator.
    backend: Box<dyn PoolBackend>,

    /// Per-slot data pointers for non-mmap backends.
    /// For MmapBackend, this is empty (pointers computed from data_base_ptr + offset).
    /// For CUDA backends, each slot's BackendAllocation pointer is stored here
    /// because the offset-from-base model doesn't apply.
    slot_data_ptrs: parking_lot::Mutex<std::collections::HashMap<u32, *mut u8>>,
...
impl TensorPool {
    ...
    pub fn new(pool_id: u32, config: TensorPoolConfig) -> HorusResult<Self> {
        ...
        // is within bounds; the PoolHeader containing next_alloc_offset is at mmap_ptr.
        // Both pointers remain valid for the lifetime of `pool` (mmap lives in the struct).
        let backend: Box<dyn PoolBackend> = unsafe {
            let data_base = mmap.as_ptr().add(data_offset) as *mut u8;
            let header_ptr = mmap.as_ptr() as *const PoolHeader;
            let next_alloc = &(*header_ptr).next_alloc_offset as *const AtomicU64;
            Box::new(MmapBackend::new(
                data_base,
                next_alloc,
                config.pool_size,
    ...
    /// Create a tensor pool with a custom memory backend.
    ///
    /// The pool metadata (PoolHeader + SlotHeaders) is always stored in mmap
    /// for cross-process discovery. The **data region** is managed by the
    /// provided backend. For GPU backends, the mmap is metadata-only (no
    /// data region in the shared file).
    ///
    /// # Example
    ///
    /// ```rust,ignore
    /// let backend = Box::new(MyCudaBackend::new(Device::cuda(0)));
    /// let pool = TensorPool::with_backend(42, TensorPoolConfig::default(), backend)?;
    /// ```
    pub fn with_backend(
        pool_id: u32,
        config: TensorPoolConfig,
        ...
        // Layout: metadata only (header + slots). Data lives in the backend.
        let header_size = std::mem::size_of::<PoolHeader>();
        let slots_size = config.max_slots * std::mem::size_of::<SlotHeader>();
        let metadata_size = header_size + slots_size;
        let data_offset = Self::align_up(metadata_size, config.slot_alignment);

        // For mmap-backed data, the file includes the data region.
        // For non-mmap backends, the file is metadata-only.
        let mmap_size = if backend.is_shared() {
            data_offset + config.pool_size
        } else {
            data_offset
        };
    ...
    fn initialize(&mut self) -> HorusResult<()> {
        ...
        // SAFETY: header pointer derived from mmap.as_mut_ptr(), valid for the pool lifetime.
        // Volatile write ensures the store is not reordered or elided by the compiler.
        unsafe {
    ...
    pub fn alloc(&self, shape: &[u64], dtype: TensorDtype, device: Device) -> HorusResult<Tensor> {
        // Soundness: ensure the caller's device matches the backend's device.
        // Without this check, the Tensor descriptor could claim CUDA while the
        // data lives in mmap (CPU) — causing segfaults when a GPU kernel
        // dereferences the pointer, or DLPack exporting a CPU pointer as CUDA.
        let backend_dev = self.backend.device();
        if device != backend_dev {
            return Err(HorusError::Memory(
                format!(
                    "device mismatch: requested {} but pool backend targets {} (backend: {}). \
                     Use a pool created with the matching backend, or pass Device::{}() to alloc().",
                    device,
                    backend_dev,
                    self.backend.name(),
                    if backend_dev.is_cpu() { "cpu" } else { "cuda" },
                )
                .into(),
            ));
        }
        ...
        let backend_alloc = match alloc_result {
            Ok(a) => a,
            Err(msg) => {
                self.push_free_slot(slot_id);
                return Err(HorusError::Memory(msg.into()));
            }
        };

        // Compute offset and store data pointer.
        //
        // For MmapBackend (is_shared=true): offset = cpu_ptr - data_base_ptr.
        // For non-mmap backends (CUDA managed/pinned/device): store the raw pointer
        // in slot_data_ptrs; data_ptr() reads from there instead of base+offset.
        let offset = if self.backend.is_shared() && !backend_alloc.cpu_ptr.is_null() {
            // SAFETY: both pointers are within the same mmap allocation.
            unsafe { backend_alloc.cpu_ptr.offset_from(self.data_base_ptr()) as usize }
        } else {
            // Non-mmap backend — store pointer in per-slot map
            let ptr = if !backend_alloc.cpu_ptr.is_null() {
                backend_alloc.cpu_ptr
            } else if !backend_alloc.device_ptr.is_null() {
                backend_alloc.device_ptr
            } else {
                self.push_free_slot(slot_id);
                return Err(HorusError::Memory(
            ...
            0 // offset is meaningless for non-mmap; pointer is in the map
        };
    ...
    pub fn data_slice(&self, tensor: &Tensor) -> HorusResult<&[u8]> {
        ...
        // Generation check — prevents reading stale data after a slot was freed
        // and reallocated. This catches the ABA race where:
        // 1. Process A spills a large message to slot X (generation G)
    ...
    pub fn refcount(&self, tensor: &Tensor) -> u64 {
        if tensor.pool_id != self.pool_id {
            return 0;
        }
    ...
    /// Get the backend name (e.g., "mmap", "cuda_managed").
    pub fn backend_name(&self) -> &str {
        self.backend.name()
    }

    /// Get the device this pool's backend targets.
    pub fn backend_device(&self) -> Device {
        self.backend.device()
    }
    ...
    fn return_slot(&self, slot_id: u32) {
        ...
        //
        // Delegated to the backend's zero() method:
        // - MmapBackend: volatile byte writes + compiler fence (default impl)
        // - GPU backends: may use cudaMemset or skip if not security-relevant
        //
        // Ordering:
        ...
        if data_size > 0 {
            ...
            if !data_ptr.is_null() {
                ...
                self.backend.zero(&alloc);
                // Free the backend allocation (no-op for mmap, actual free for CUDA)
                self.backend.free(&alloc);
            }
        }

        // Remove from slot data pointer map (non-mmap backends)
        if !self.backend.is_shared() {
            self.slot_data_ptrs.lock().remove(&slot_id);
        }
...
mod tests {
    ...
    fn test_concurrent_retain_release() {
        ...
        assert_eq!(
            ...
            1 + (num_threads * ops_per_thread) as u64
        );
    ...
    fn test_pool_alloc_when_full_returns_error() {
        ...
        // Next alloc should fail (pool full — either slots or data exhausted)
        let result = pool.alloc(&[100], TensorDtype::U8, Device::cpu());
        assert!(result.is_err(), "alloc on full pool should return error");

        // Note: TensorPool uses a bump allocator for the data region.
        // Releasing a slot returns it to the free stack but does NOT reclaim
        // the data offset.  A freed slot can only be reused if the data region
        // still has room for a new allocation.  In a small pool that's already
        // at capacity, release + re-alloc will fail because there's no space
        // left in the data region even though the slot is free.
        //
        // This is by design: the bump allocator avoids fragmentation and is
        // lock-free, which matters for real-time workloads.
    }
    ...
    fn test_stats_accuracy_alloc_n_release_m() {
        ...
        assert_eq!(stats.total_refcount, n as u64);
        assert!(stats.used_bytes > 0);
        ...
        assert_eq!(stats.total_refcount, (n - m) as u64);
