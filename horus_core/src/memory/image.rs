...
impl Image {
    ...
    ///
    /// The device is inferred from the pool's backend. Use this for
    /// GPU-backed images (pool created with a CUDA backend).
    pub fn new_on(
        width: u32,
        height: u32,
    ...
    ///
    /// If the image is already on CPU, returns a clone (refcount bump, no copy).
    pub fn to_cpu(&self) -> HorusResult<Self> {
        if self.is_cpu() {
            return Ok(self.clone());
        }

        // Create a new image on the global CPU pool
        let mut cpu_img = Self::new(self.width(), self.height(), self.encoding())?;

        // Copy data: GPU → CPU
        // For managed memory, data() works (CPU-accessible).
        // For device-only memory, data() returns error — need explicit memcpy.
        let src_data = self.data();
        let dst_data = cpu_img.data_mut();
        dst_data.copy_from_slice(src_data);

        cpu_img.set_timestamp_ns(self.timestamp_ns());
        cpu_img.set_frame_id(self.frame_id());
    ...
    pub fn pixel(&self, x: u32, y: u32) -> Option<&[u8]> {
        ...
        let offset = y as usize * self.step() as usize
            + x as usize * self.encoding().bytes_per_pixel() as usize;
        let data = self.data();
