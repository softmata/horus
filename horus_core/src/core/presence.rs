...
pub struct NodePresence {
    ...
    #[serde(default)]
    actions: Vec<String>,
    /// Actual SCHED_FIFO priority applied (None = normal priority)
    #[serde(default)]
    rt_priority: Option<i32>,
    /// CPU core this node's thread is pinned to (None = unpinned)
    #[serde(default)]
    pinned_core: Option<usize>,
    /// Whether memory is locked (mlockall succeeded)
    #[serde(default)]
    memory_locked: bool,
    /// Whether PREEMPT_RT kernel is active
    #[serde(default)]
    preempt_rt: bool,
    /// Tick budget in microseconds (None = no budget)
    #[serde(default)]
    budget_us: Option<u64>,
    /// Deadline in microseconds (None = no deadline)
    #[serde(default)]
...
impl NodePresence {
    ...
    /// Update health status for live presence refresh.
    pub fn set_health_status(&mut self, status: &str) {
    ...
    pub fn rt_degradations(&self) -> &[String] {
    ...
    pub fn set_rt_state(
        ...
        self.pinned_core = core;
        self.memory_locked = mem_locked;
        self.preempt_rt = preempt;
        self.budget_us = budget;
        self.deadline_us = deadline;
        self.rt_degradations = degradations;
    }
}
...
impl NodePresence {
    ...
    pub(crate) fn new(
        ...
        // Derive action names from topics: if this node publishes/subscribes to
        // topics matching the action naming convention ({name}.goal, {name}.result,
        // etc.), record the action name so `horus action list` discovers it.
        let actions = Self::derive_actions_from_topics(&publishers, &subscribers);

        // Similarly derive service names from .request/.response topics.
        let services = Self::derive_services_from_topics(&publishers, &subscribers);
    ...
    /// Derive action names from publisher/subscriber topic lists.
    ///
    /// Looks for topics matching `{name}.goal`, `{name}.result`, `{name}.feedback`,
    /// `{name}.status`, `{name}.cancel`. An action name is included if at least
    /// one of these topics is present.
    fn derive_actions_from_topics(
        publishers: &[TopicMetadata],
        subscribers: &[TopicMetadata],
    ) -> Vec<String> {
        let action_suffixes = [".goal", ".result", ".feedback", ".status", ".cancel"];
        let mut action_names = std::collections::BTreeSet::new();
        for topic in publishers.iter().chain(subscribers.iter()) {
            for suffix in &action_suffixes {
                if let Some(action_name) = topic.topic_name.strip_suffix(suffix) {
    ...
    /// Derive service names from publisher/subscriber topic lists.
    ///
    /// Looks for topics matching `{name}.request` or `{name}.response`.
    fn derive_services_from_topics(
        publishers: &[TopicMetadata],
        subscribers: &[TopicMetadata],
    ) -> Vec<String> {
        let service_suffixes = [".request", ".response"];
        let mut service_names = std::collections::BTreeSet::new();
        for topic in publishers.iter().chain(subscribers.iter()) {
            for suffix in &service_suffixes {
                if let Some(svc_name) = topic.topic_name.strip_suffix(suffix) {
                    if !svc_name.is_empty() {
                        service_names.insert(svc_name.to_string());
                    }
                }
            }
        }
