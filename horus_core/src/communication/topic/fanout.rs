...
impl<T: Clone> FanoutRing<T> {
    ...
    pub fn new(max_pubs: usize, max_subs: usize, channel_capacity: usize) -> Self {
        let max_pubs = max_pubs.min(MAX_ENDPOINTS).max(1);
        let max_subs = max_subs.min(MAX_ENDPOINTS).max(1);
    ...
    pub fn send_as(&self, msg: T, pub_id: usize) -> Result<(), T> {
        ...
        // Try to send to all subscriber channels
        // If any channel is full, we still send to the others (lossy fan-out)
        for sub_id in 0..n_subs {
            let _ = pub_channels[sub_id].try_send(msg.clone());
        }
