...
struct KernelApi {
    color_convert:
        unsafe extern "C" fn(*const u8, *mut u8, i32, i32, i32, i32, *mut c_void) -> KernelResult,
    ...
    transpose_hwc_to_chw: unsafe extern "C" fn(
        *const c_void,
        *mut c_void,
        i32,
        i32,
    ...
    transpose_chw_to_hwc: unsafe extern "C" fn(
        *const c_void,
        *mut c_void,
        i32,
        i32,
    ...
    preprocess_fused: unsafe extern "C" fn(
        *const u8,
        *mut f32,
        i32,
        i32,
...
fn load_kernel_api() -> Result<KernelApi, String> {
    ...
    let search_paths = [
        ...
        concat!(
            ...
            "/src/gpu/kernels/libhorus_kernels.so"
        ),
        // Installed location
...
pub fn color_convert(
    ...
    let k = api()?;
    check_kernel(
        unsafe { (k.color_convert)(src, dst, width, height, src_fmt, dst_fmt, stream) },
        "color_convert",
    )
}
...
pub fn resize(
    ...
    let k = api()?;
    check_kernel(
        unsafe { (k.resize)(src, dst, src_w, src_h, dst_w, dst_h, channels, stream) },
        "resize",
    )
}
...
pub fn transpose_hwc_to_chw(
    ...
    let k = api()?;
    check_kernel(
        unsafe { (k.transpose_hwc_to_chw)(src, dst, width, height, channels, elem_size, stream) },
...
pub fn transpose_chw_to_hwc(
    ...
    let k = api()?;
    check_kernel(
        unsafe { (k.transpose_chw_to_hwc)(src, dst, width, height, channels, elem_size, stream) },
        "transpose_chw_to_hwc",
    )
}
...
pub fn crop_pad(
    ...
    check_kernel(
        unsafe {
            (k.crop_pad)(
                src, dst, src_w, src_h, crop_x, crop_y, crop_w, crop_h, dst_w, dst_h, channels,
                pad_value, stream,
            )
        },
        "crop_pad",
...
mod tests {
    ...
    fn test_kernels_load() {
        ...
        assert!(
            kernels_available(),
            "libhorus_kernels.so should be loadable"
        );
    }
    ...
    fn test_fused_preprocess() {
        ...
        let src_ptr = src_dptr as *mut u8;
        unsafe {
            std::ptr::write_bytes(src_ptr, 128, src_size);
        }
