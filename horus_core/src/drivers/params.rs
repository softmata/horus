//! Typed access to driver configuration parameters.
//!
//! [`DriverParams`] wraps the `HashMap<String, toml::Value>` from a
//! `[drivers.NAME]` table in `horus.toml` and provides typed getters.
//!
//! # Example
//!
//! ```rust,ignore
...
//!
//! fn create_driver(params: &DriverParams) -> HorusResult<MyDriver> {
...
///
/// Wraps the flattened params HashMap from a `[drivers.NAME]` table.
...
impl DriverParams {
    /// Create from a raw params map (typically from `DriverTableConfig.params`).
