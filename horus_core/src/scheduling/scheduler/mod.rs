...
// =========================================================================
// Global network auto-wire hook
// =========================================================================
//
// The `horus` umbrella crate registers a function here (via `set_network_auto_wire`)
// that starts the `horus_net` replicator. The scheduler calls it automatically in
// `run_with_filter()` when `network_enabled` is true, so users never need to call
// `horus::net::wire()` manually.
//
// This avoids a circular dependency: horus_net depends on horus_core, so horus_core
// cannot depend on horus_net. Instead, the umbrella crate bridges the gap.

type NetworkAutoWireFn = Box<dyn Fn(&mut Scheduler) + Send + Sync>;
static NETWORK_AUTO_WIRE: OnceLock<NetworkAutoWireFn> = OnceLock::new();

/// Register the function that auto-starts network replication.
///
/// Called once by the `horus` umbrella crate (when the `net` feature is enabled).
/// The registered function is invoked automatically at the start of `Scheduler::run()`
/// when `network_enabled` is true, eliminating the need for users to call
/// `horus::net::wire()` manually.
///
/// Can only be set once (first caller wins). Thread-safe.
pub fn set_network_auto_wire(f: impl Fn(&mut Scheduler) + Send + Sync + 'static) {
...
/// A startup hook returns an opaque handle. The handle is held for the scheduler's
...
pub struct Scheduler {
    ...
    /// Shared node control map — set by control topic, read by executor threads.
    node_controls: Option<Arc<super::types::NodeControlMap>>,
    /// Optional runtime parameter store — enables live param changes + node callbacks.
    pub(super) params: Option<Arc<crate::params::RuntimeParams>>,

    /// Whether network replication is requested. Checked by the `horus` umbrella crate
    /// (or any integration layer) via `network_enabled()`. The scheduler itself does not
    /// depend on `horus_net` — the integration layer registers a lifecycle hook.
    network_enabled: bool,

    /// Lifecycle startup hooks — invoked once at the start of `run_with_filter()`, after
    /// signal handlers are set up. Each hook returns an optional opaque handle; handles
    /// are held until shutdown and dropped in reverse order (LIFO).
    lifecycle_start_hooks: Vec<LifecycleStartFn>,

    /// Handles returned by lifecycle startup hooks. Dropped during shutdown in reverse
    /// order (LIFO) so the last hook started is the first stopped.
    lifecycle_handles: Vec<Box<dyn Any + Send>>,
...
impl Scheduler {
    ...
    pub fn new() -> Self {
        ...
        let mut s = Self {
            nodes: Vec::new(),
            running,
            scheduler_name: std::env::var("HORUS_NODE_NAME")
                .ok()
                .filter(|s| !s.is_empty())
    ...
    pub fn require_rt(mut self) -> Self {
        ...
        self.pending_config.realtime.memory_locking = true;
        self.pending_config.realtime.rt_scheduling_class = true;
        self.pending_config.realtime.require_mode = true;
        self
    }
    ...
    ///
    /// Network is **on by default**. Use `.network(false)` to opt out.
    /// This overrides the `HORUS_NET_ENABLED` env var in either direction.
    pub fn network(mut self, enabled: bool) -> Self {
        self.network_enabled = enabled;
        self
    }

    /// Check whether network replication is enabled.
    ///
    /// Returns `true` by default. Returns `false` if `.network(false)` was called
    /// or `HORUS_NET_ENABLED=false` env var is set.
    pub fn network_enabled(&self) -> bool {
        self.network_enabled
    }

    /// Register a lifecycle hook that runs once at scheduler startup.
    ///
    /// The hook is invoked after signal handlers are set up but before the tick loop
    /// begins. It may return an opaque handle (`Box<dyn Any + Send>`) that is held
    /// for the scheduler's lifetime and dropped during shutdown in reverse order.
    ///
    /// This is the integration point for `horus_net` and other subsystems that need
    /// to run background threads alongside the scheduler without introducing circular
    /// dependencies.
    ///
    /// # Example
    /// ```rust,ignore
    ...
    pub fn apply_config(&mut self, config: super::config::SchedulerConfig) {
        ...
        if config.realtime.require_mode && !self.has_full_rt() {
            let failed: Vec<_> = self
                .rt
                .degradations
                .iter()
                .filter(|d| d.severity == DegradationSeverity::High)
                .map(|d| format!("{}: {}", d.feature, d.reason))
                .collect();
            if !failed.is_empty() {
                print_line(&format!(
            ...
            match rt_config.apply() {
                ...
                Ok(RtApplyResult::Degraded(ref degradations)) => {
                    ...
                    for d in degradations {
                        ...
                        let (feature, severity) = match d {
                            RtDegradation::NoPreemptRt => {
                                (RtFeature::RtPriority, DegradationSeverity::Medium)
                            }
                            RtDegradation::MemoryLockUnavailable(_) => {
                                (RtFeature::MemoryLocking, DegradationSeverity::High)
                            }
                            RtDegradation::SchedulerDegraded(_) => {
                                (RtFeature::RtPriority, DegradationSeverity::High)
                            }
                            RtDegradation::PriorityClamped { .. } => {
                                (RtFeature::RtPriority, DegradationSeverity::Low)
                            }
                            RtDegradation::AffinityUnavailable(_) => {
                                (RtFeature::CpuAffinity, DegradationSeverity::Medium)
                            }
                        };
    ...
    fn add_configured_internal(
        ...
        self.nodes.push(RegisteredNode {
            ...
            subscription_freshness: config.subscription_freshness,
            use_sched_deadline: config.use_sched_deadline,
        ...
        } else if self.pending_config.monitoring.verbose {
            print_line(&format!(
                "Added {} '{}' with priority {}",
                if is_rt_node { "RT node" } else { "node" },
                node_name,
    ...
    fn finalize_and_init(&mut self) {
        ...
        // thread, including CLI), nodes with on_parameter_change() are notified.
        // Note: this is a synchronous callback on the set() caller's thread, not deferred
        // to the next tick. For tick-deferred notification, use a Topic-based approach.
        if let Some(ref params) = self.params {
            // We can't hold &mut self.nodes in a closure registered on params (which is Arc).
            // Instead, we just log that params are attached. The actual node notification
            // happens when the user calls params.set() — the on_change callback they
            // registered can call node methods directly if they hold a reference.
            //
            // For full scheduler-mediated routing, a param change Topic would be needed
            // (future work — the on_change() mechanism on RuntimeParams is the foundation).
            let _ = params; // Params are attached and accessible via self.params
        }
        ...
        if self.rt_require_failed {
            return Err(crate::error::Error::Resource(
                crate::error::ResourceError::Unsupported {
                    ...
                    reason: "require_rt() failed: high-severity RT degradations occurred. \
                             Use .prefer_rt() for graceful degradation."
                        .into(),
                },
            ));
        }
    ...
    fn execute_single_tick(&mut self, node_filter: Option<&[&str]>) -> HorusResult<()> {
        ...
        // Periodically update presence files with live tick counts (every 100 ticks).
        // This ensures `horus node list` shows current tick counts and rate,
        // especially in deterministic mode where the SHM SchedulerRegistry isn't used.
        ...
        if self.tick.current == 1 || self.tick.current.is_multiple_of(ticks_per_sec) {
            for node in &self.nodes {
                if node.initialized && !node.is_stopped {
                    ...
                    // Topic associations come from TopicNodeRegistry (auto-populated by Topic::new())
                    ...
                    if let Ok(mut p) = NodePresence::new(
                        ...
                        p.set_health_status(health);
                        // Populate RT state from scheduler
    ...
    fn run_with_filter(
        ...
        rt.block_on(async {
            ...
            //   - `self.network_enabled` is false (.network(false) or HORUS_NET_ENABLED=false)
            //   - No auto-wire function was registered (using horus_core directly without horus umbrella)
            //   - A lifecycle hook was already registered manually (e.g., via horus::net::wire_with_config)
            if self.network_enabled && self.lifecycle_start_hooks.is_empty() {
                if let Some(auto_wire) = NETWORK_AUTO_WIRE.get() {
                    auto_wire(self);
                }
            }

            // Invoke lifecycle startup hooks (e.g., horus_net replicator).
            // Hooks run after signal handlers so Ctrl+C still works during startup.
            // Returned handles are held until shutdown, dropped in reverse order.
            let hooks = std::mem::take(&mut self.lifecycle_start_hooks);
            for hook in hooks {
                if let Some(handle) = hook() {
                    self.lifecycle_handles.push(handle);
                }
            }
            ...
            if deterministic {
                ...
                self.node_controls = Some(arc_controls.clone());
            ...
            // started is the first stopped. This stops horus_net's replicator
            // thread (ReplicatorHandle::drop signals stop + joins with 3s timeout).
            while let Some(handle) = self.lifecycle_handles.pop() {
                drop(handle);
            }
                ...
                if let Some(ref mut ctx) = registered.context {
                    ...
                    match init_result {
                        Ok(()) => {
                            ...
                            // Topic associations come from TopicNodeRegistry
                            // (may be empty at init — populated as Topic::new() is called during ticks)
                            let registry = crate::communication::topic_node_registry();
                            let publishers = registry.publishers_for_node(node_name);
                            let subscribers = registry.subscribers_for_node(node_name);
    ...
    fn periodic_monitoring(&mut self, start_time: Instant) {
        ...
        if let Some(ref bb) = self.monitor.blackbox {
            bb.lock().unwrap_or_else(|p| p.into_inner()).tick();
        }
    ...
    fn finalize_run(&mut self) {
        ...
        // Flush recording data to disk if recording was enabled.
        // This is how `horus run --record <session>` persists captured data.
        if self.recording.is_some() {
            match self.stop_recording() {
                Ok(paths) if !paths.is_empty() => {
                    print_line(&format!(
                        "[RECORDING] Session saved ({} files)",
                        paths.len()
                    ));
                }
                Err(e) => {
                    print_line(&format!("[RECORDING] Failed to save: {}", e));
                }
                _ => {}
            }
        }
    ...
    fn print_timing_report(&self) {
        ...
        for name in &node_names {
            ...
            eprintln!(
                "{}",
                format_timing_report_row(name, stats, safety_data.get(*name))
            );
    ...
    fn update_registry(&self) {
        if let Ok(registry_path) = Self::get_registry_path() {
            ...
            let nodes_json: Vec<String> = self.nodes.iter().map(|registered| {
                ...
                // Get pub/sub from TopicNodeRegistry (auto-populated by Topic::new())
                let tnr = crate::communication::topic_node_registry();
                let publishers = tnr.publishers_for_node(name);
                let subscribers = tnr.subscribers_for_node(name);
    ...
    fn process_tick_result(
        ...
        match tick_result {
            Ok(_) => {
                ...
                // Subscription freshness check: detect stale input topics
                // Collect stale actions first (avoid borrow conflict with node mutation)
                let mut stale_action: Option<super::types::StalePolicy> = None;
                if !self.nodes[i].subscription_freshness.is_empty() {
                    ...
                    for sf in &self.nodes[i].subscription_freshness {
                        ...
                        if elapsed_ns > timeout_ns {
                            let node_name = &*self.nodes[i].name;
                            match sf.policy {
                                super::types::StalePolicy::Warn => {
                                    crate::hlog!(
                                        warn,
                                        "Subscription stale: topic '{}' on node '{}' — no data for {:.0}ms",
                                ...
                                | super::types::StalePolicy::Stop) => {
                                    crate::hlog!(
                                        ...
                                        sf.topic, node_name, p
                                    );
                                    stale_action = Some(p);
                                    break; // One action per tick is enough
                                }
                            }
                        }
                    }
                }
                // Apply stale action (separate borrow scope)
