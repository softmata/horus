...
// ── Global blackbox hook for external systems (horus_net) ──────────────────

type BlackBoxHook = Box<dyn Fn(BlackBoxEvent) + Send + Sync>;
static BLACKBOX_HOOK: std::sync::OnceLock<BlackBoxHook> = std::sync::OnceLock::new();

/// Set a global hook to record blackbox events from external systems.
/// Called by the Scheduler at startup to wire the blackbox.
pub fn set_blackbox_hook(hook: impl Fn(BlackBoxEvent) + Send + Sync + 'static) {
    let _ = BLACKBOX_HOOK.set(Box::new(hook));
}

/// Record a blackbox event from an external system (e.g., horus_net).
/// If the hook is set, the event is recorded to the flight recorder.
pub fn record_external_event(event: BlackBoxEvent) {
    if let Some(hook) = BLACKBOX_HOOK.get() {
        hook(event);
    }
}
...
pub enum BlackBoxEvent {
    ...
    NetReplicationStarted { peer_count: usize },
    /// Network import rejected by guard
    NetImportRejected { topic: String, peer_addr: String },
    /// Custom event
