...
pub struct NodeSlot {
    ...
    /// P99 tick duration (nanoseconds) from TickTimingRing.
    pub p99_tick_ns: AtomicU64,
    /// Padding.
    pub _pad2: [u8; 32],
}
...
impl SchedulerRegistry {
    ...
    pub fn register_node(&self, name: &str, order: u8, rate_hz: f64, execution_class: u8) -> usize {
        ...
        unsafe {
            ...
            *slot.add(51) = order;
            ...
            (*(slot.add(152) as *const AtomicU64)).store(0, Ordering::Release); // p99_tick_ns
    ...
    pub fn update_node(
        ...
        unsafe {
            ...
            (*(slot.add(84) as *const AtomicU8)).store(watchdog_severity, Ordering::Relaxed);
            ...
            (*(slot.add(152) as *const AtomicU64)).store(p99_tick_ns, Ordering::Relaxed);
        }
    }
    ...
    pub fn read_all_slots_from_path(path: &std::path::Path) -> Option<Vec<NodeSlotSnapshot>> {
        ...
        // SAFETY: base points to valid SHM region (validated by magic check above).
        // Offset 12 is the node_count field in the registry header (magic:u64 + version:u32 = 12).
        let node_count =
            unsafe { (*(base.add(12) as *const AtomicU32)).load(Ordering::Acquire) } as usize;
        ...
        for i in 0..node_count {
            ...
            unsafe {
                ...
                let p99_tick_ns = (*(slot.add(152) as *const AtomicU64)).load(Ordering::Relaxed);
...
mod tests {
    ...
    fn update_node_invalid_index_ignored() {
        ...
        reg.update_node(999, 0, 100, 0, 0, 0, 0, 0, 0, 0, 0);
        reg.update_node(MAX_REGISTRY_NODES, 0, 100, 0, 0, 0, 0, 0, 0, 0, 0);
