...
fn generate_rt_readiness_report() {
    ...
    // Basic sanity checks
    assert!(report.jitter_samples > 1000, "Should have >1000 jitter samples");
    assert!(report.ipc_latency_ns > 0.0, "IPC latency should be measured");
    assert!(report.ipc_throughput_msg_per_sec > 1000.0, "IPC should do >1000 msg/sec");
