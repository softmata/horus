...
fn dc_motor_pid_1khz_position_control() {
    ...
    if n > 0 {
        ...
        let settled_idx = log.iter().enumerate().position(|(i, &p)| {
            if (p - target_position).abs() > threshold {
                return false;
            }
            ...
            log[i..]
                .iter()
                .take(100)
                .all(|&v| (v - target_position).abs() < threshold)
        });
        ...
        println!(
            "Final position: {:.4} rad (error: {:.4} rad, {:.2}%)",
        ...
        if let Some(idx) = settled_idx {
            ...
            assert!(
                settling_ms < 2000.0,
                "Motor should settle within 2s, took {:.0}ms",
                settling_ms
            );
        } else {
        ...
        assert!(
            final_error_pct < 5.0,
            "Final error {:.2}% exceeds 5% threshold",
            final_error_pct
        );
        ...
        assert!(
            overshoot_pct < 30.0,
            "Overshoot {:.2}% exceeds 30% threshold",
            overshoot_pct
        );
...
fn sensor_suite_imu400_lidar10_camera30() {
    ...
    let ls = lidar_sent.clone();
    let cs = camera_sent.clone();
    let ir = imu_recv.clone();
    let lr = lidar_recv.clone();
    let cr = camera_recv.clone();
    ...
    let handle = std::thread::spawn(move || {
        ...
        let _ = sched
            .add(CameraSensorNode {
                topic_name: camera_topic.clone(),
                topic: None,
                seq: 0,
                tick_count: cs,
            })
    ...
    println!(
        "IMU    (400Hz): sent={:5}, recv={:5} ({:.0}%)",
        is,
        ir,
        if is > 0 {
            ir as f64 / is as f64 * 100.0
        } else {
            0.0
        }
    );
    println!(
        "LiDAR   (10Hz): sent={:5}, recv={:5} ({:.0}%) [{} bytes/scan]",
        ls,
        lr,
        if ls > 0 {
            lr as f64 / ls as f64 * 100.0
        } else {
            0.0
        },
        std::mem::size_of::<LidarScan>()
    );
    println!(
        "Camera  (30Hz): sent={:5}, recv={:5} ({:.0}%) [1024 bytes/frame]",
        cs,
        cr,
        if cs > 0 {
            cr as f64 / cs as f64 * 100.0
        } else {
            0.0
        }
    );
    ...
    assert!(
        ir > 50,
        "Fusion should receive >50 IMU messages, got {}",
        ir
    );
...
fn io_contention_rt_resilience() {
    ...
    assert!(
        actual_hz > target_hz as f64 * 0.3,
        "Rate {:.1}Hz is less than 30% of target {}Hz under I/O stress",
        actual_hz,
        target_hz
    );
    ...
    assert!(
        max < 100_000.0,
        "Max period {:.0}μs exceeds 100ms — scheduler stalled",
        max
    );
...
fn memory_pressure_no_hot_path_allocations() {
    ...
    println!(
        "RSS growth: {} KB ({:.2} bytes/tick)",
        rss_growth,
        rss_growth as f64 * 1024.0 / ticks as f64
    );

    // Growth should be < 1MB for 50K ticks (< 20 bytes/tick)
    assert!(
        rss_growth < 1024,
        "RSS grew {} KB during {} ticks — hot path is allocating",
        rss_growth,
        ticks
    );

    println!(
        "MEMORY PRESSURE TEST PASSED ✓ (< 1MB growth over {} ticks)",
        ticks
    );
}
