...
mod common;
use common::{cleanup_stale_shm, timestamp_now};
...
fn chain_world_base_arm_tool() {
    ...
    tf.update_transform(
        "tool_link",
        &Transform::from_translation([0.0, 0.0, 1.0]),
        timestamp_now(),
    )
    .unwrap();
    ...
    assert!(
        (result.translation[0] - 1.0).abs() < 0.01,
        "x: {}",
        result.translation[0]
    );
    assert!(
        (result.translation[1] - 1.0).abs() < 0.01,
        "y: {}",
        result.translation[1]
    );
    assert!(
        (result.translation[2] - 1.0).abs() < 0.01,
        "z: {}",
    ...
    let point = tf
        .transform_point("tool_link", "world", [0.0, 0.0, 0.0])
        .unwrap();
...
fn deep_hierarchy_20_frames() {
    ...
    for i in 1..20 {
        tf.register_frame(&format!("link_{}", i), Some(&format!("link_{}", i - 1)))
            .unwrap();
        tf.update_transform(
            &format!("link_{}", i),
    ...
    println!(
        "link_19→link_0 translation: {:?} (expected x={:.1})",
        result.translation, expected_x
    );

    assert!(
        (result.translation[0] - expected_x).abs() < 0.01,
        "20-hop chain: expected x={}, got {}",
        expected_x,
        result.translation[0]
    );
...
fn concurrent_writers_no_corruption() {
    ...
    tf.update_transform(
        "left_wheel",
        &Transform::from_translation([-0.15, 0.0, 0.0]),
        timestamp_now(),
    )
    .unwrap();
    tf.update_transform(
    ...
    println!(
        "Concurrent: {} reads, {} writes, {} NaN/Inf",
        reads,
        w1 + w2,
        bad
    );
    assert_eq!(
        bad, 0,
        "Concurrent writes produced NaN/Inf in {} reads",
        bad
    );
    assert!(reads > 1000, "Should read >1000 transforms in 3s");
    println!(
        "✓ concurrent_writers_no_corruption — {} reads, zero NaN/Inf",
        reads
    );
}
...
fn cross_scheduler_transform_sharing() {
    ...
    let h1 = std::thread::spawn(move || {
        ...
        let _ = sched
            .add(TfWriterNode { tf: tf1, tick: 0 })
            .rate(200_u64.hz())
            .order(0)
            .build();
        while r1.load(Ordering::Relaxed) {
    ...
    println!(
        "Cross-scheduler TF: {} reads, {} valid ({:.0}%)",
        reads,
        valid,
        valid as f64 / reads.max(1) as f64 * 100.0
    );
    assert!(reads > 50, "Reader should get >50 transform lookups");
    assert_eq!(
        reads, valid,
        "All reads should return valid (finite, non-negative) transforms"
    );
    println!(
        "✓ cross_scheduler_transform_sharing — {} reads, 100% valid",
        reads
    );
}
