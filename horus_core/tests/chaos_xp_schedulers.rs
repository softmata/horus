...
fn read_result(id: u64, key: &str) -> String {
    std::fs::read_to_string(result_dir().join(format!("{}_{}", id, key)))
        .unwrap_or_default()
}

fn spawn(test: &str, topic: &str, role: &str, dur_ms: u64, rate: u64, id: u64) -> std::process::Child {
    let exe = std::env::current_exe().unwrap();
    Command::new(exe)
        .args([test, "--exact", "--nocapture", "--ignored"])
        .env(CHILD, "1").env(TOPIC_ENV, topic).env(ROLE_ENV, role)
...
impl Node for ControllerNode {
    fn name(&self) -> &str { "xps_controller" }
    fn init(&mut self) -> horus_core::error::HorusResult<()> {
...
fn child_sched_imu_pub() {
    ...
    let _ = sched.add(ImuPubNode {
        topic: None, topic_name: topic, seq: 0, published: published.clone(),
    }).rate(rate.hz()).order(0).build();
    let _ = sched.run_for(Duration::from_millis(dur_ms));

    write_result(id, "published", &published.load(Ordering::Relaxed).to_string());
}
...
fn child_sched_controller() {
    ...
    let _ = sched.add(ControllerNode {
        imu_name: imu_topic, cmd_name: cmd_topic,
        imu_topic: None, cmd_topic: None,
        imu_recv: imu_recv.clone(), cmd_sent: cmd_sent.clone(), corrupted: corrupted.clone(),
    }).rate(rate.hz()).order(0).build();
    let _ = sched.run_for(Duration::from_millis(dur_ms));

    write_result(id, "imu_recv", &imu_recv.load(Ordering::Relaxed).to_string());
    write_result(id, "cmd_sent", &cmd_sent.load(Ordering::Relaxed).to_string());
    write_result(id, "corrupt", &corrupted.load(Ordering::Relaxed).to_string());
}
...
fn child_sched_motor() {
    ...
    let _ = sched.add(MotorNode {
        cmd_name: cmd_topic, cmd_topic: None,
        cmd_recv: cmd_recv.clone(), corrupted: corrupted.clone(),
    }).rate(rate.hz()).order(0).build();
    let _ = sched.run_for(Duration::from_millis(dur_ms));

    write_result(id, "cmd_recv", &cmd_recv.load(Ordering::Relaxed).to_string());
    write_result(id, "corrupt", &corrupted.load(Ordering::Relaxed).to_string());
}
...
fn child_sched_imu_sub() {
    ...
    let mut sched = Scheduler::new().tick_rate(rate.hz()).name(&format!("sub_{}_sched", id));
    let _ = sched.add(ImuSubNode {
        topic: None, topic_name: topic,
        received: received.clone(), corrupted: corrupted.clone(),
    }).rate(rate.hz()).order(0).build();
    let _ = sched.run_for(Duration::from_millis(dur_ms));

    write_result(id, "received", &received.load(Ordering::Relaxed).to_string());
    write_result(id, "corrupt", &corrupted.load(Ordering::Relaxed).to_string());
}
...
fn xps_3_process_pipeline_with_schedulers() {
    ...
    let mut motor = spawn("xps_3_process_pipeline_with_schedulers", &topic, "sched_motor", dur + 1000, 100, 20);
    std::thread::sleep(Duration::from_millis(300));
    let mut ctrl = spawn("xps_3_process_pipeline_with_schedulers", &topic, "sched_ctrl", dur + 500, 100, 10);
    std::thread::sleep(Duration::from_millis(300));
    let mut pub_child = spawn("xps_3_process_pipeline_with_schedulers", &topic, "sched_imu_pub", dur, 200, 1);
    ...
    println!("╠══════════════════════════════════════════════════════════════╣");
    println!("║  Process A (Sched 200Hz): published {:5} Imu               ║", published);
    println!("║  Process B (Sched 100Hz): recv {:5} Imu → sent {:5} Cmd   ║", ctrl_imu, ctrl_cmd);
    println!("║  Process C (Sched 100Hz): recv {:5} CmdVel                 ║", motor_cmd);
    println!("║  Corruption: ctrl={} motor={}                               ║", ctrl_corrupt, motor_corrupt);
    println!("╚══════════════════════════════════════════════════════════════╝");

    assert_eq!(ctrl_corrupt + motor_corrupt, 0, "DATA CORRUPTION in scheduler pipeline!");
...
fn xps_6_process_broadcast_mixed_rates() {
    ...
    for (i, rate) in rates.iter().enumerate() {
        subs.push(spawn("xps_6_process_broadcast_mixed_rates", &topic,
                         "sched_imu_sub", dur + 1000, *rate, 100 + i as u64));
    }
    std::thread::sleep(Duration::from_millis(500));

    // Publisher at 200Hz
    let mut pub_child = spawn("xps_6_process_broadcast_mixed_rates", &topic,
                               "sched_imu_pub", dur, 200, 1);
    ...
    println!("╠══════════════════════════════════════════════════════════════╣");
    println!("║  Published: {:5} Imu                                       ║", published);
    ...
    for (i, rate) in rates.iter().enumerate() {
        ...
        println!("║  Sub {} ({:3}Hz): recv={:5} ({:5.1}%) corrupt={}             ║",
                 i, rate, recv, recv as f64 / published.max(1) as f64 * 100.0, corrupt);
    }
    println!("╚══════════════════════════════════════════════════════════════╝");

    assert_eq!(total_corrupt, 0, "Corruption in broadcast!");
    assert!(published > 200, "Publisher should send >200");
    let subs_ok = rates.iter().enumerate().filter(|(i, _)| {
        read_result(100 + *i as u64, "received").trim().parse::<u64>().unwrap_or(0) > 0
    }).count();
    assert!(subs_ok >= 3, "At least 3/5 subs should get data, got {}", subs_ok);
    println!("✓ xps_6_process_broadcast — {} recv, zero corruption, {}/5 subs active", total_recv, subs_ok);
}
...
fn child_sched_bidir_a() {
    ...
    struct BiPubNode { t: Option<Topic<Imu>>, n: String, c: Arc<AtomicU64> }
    impl Node for BiPubNode {
        fn name(&self) -> &str { "bidir_imu_pub" }
        fn init(&mut self) -> horus_core::error::HorusResult<()> { self.t = Some(Topic::new(&self.n)?); Ok(()) }
        fn tick(&mut self) {
            let mut imu = Imu::new(); imu.linear_acceleration[2] = 9.81;
            if let Some(ref t) = self.t { t.send(imu); }
            self.c.fetch_add(1, Ordering::Relaxed);
        }
    }
    struct BiSubNode { t: Option<Topic<CmdVel>>, n: String, r: Arc<AtomicU64>, c: Arc<AtomicU64> }
    impl Node for BiSubNode {
        fn name(&self) -> &str { "bidir_cmd_sub" }
        fn init(&mut self) -> horus_core::error::HorusResult<()> { self.t = Some(Topic::new(&self.n)?); Ok(()) }
        fn tick(&mut self) {
            if let Some(ref t) = self.t {
                while let Some(cmd) = t.recv() {
                    if !cmd.linear.is_finite() { self.c.fetch_add(1, Ordering::Relaxed); }
    ...
    let _ = sched.add(BiPubNode { t: None, n: imu_topic, c: pub_count.clone() })
        .rate(100_u64.hz()).order(0).build();
    let _ = sched.add(BiSubNode { t: None, n: cmd_topic, r: recv_count.clone(), c: corrupt.clone() })
...
fn child_sched_bidir_b() {
    ...
    struct BiSubImuNode { t: Option<Topic<Imu>>, n: String, r: Arc<AtomicU64>, c: Arc<AtomicU64> }
    impl Node for BiSubImuNode {
        fn name(&self) -> &str { "bidir_imu_sub" }
        fn init(&mut self) -> horus_core::error::HorusResult<()> { self.t = Some(Topic::new(&self.n)?); Ok(()) }
        fn tick(&mut self) {
            if let Some(ref t) = self.t {
                while let Some(imu) = t.recv() {
                    if (imu.linear_acceleration[2] - 9.81).abs() > 0.01 { self.c.fetch_add(1, Ordering::Relaxed); }
                    self.r.fetch_add(1, Ordering::Relaxed);
                }
            }
        }
    }
    struct BiPubCmdNode { t: Option<Topic<CmdVel>>, n: String, c: Arc<AtomicU64> }
    impl Node for BiPubCmdNode {
        fn name(&self) -> &str { "bidir_cmd_pub" }
        fn init(&mut self) -> horus_core::error::HorusResult<()> { self.t = Some(Topic::new(&self.n)?); Ok(()) }
        fn tick(&mut self) {
    ...
    let _ = sched.add(BiSubImuNode { t: None, n: imu_topic, r: recv_count.clone(), c: corrupt.clone() })
        .rate(100_u64.hz()).order(0).build();
    let _ = sched.add(BiPubCmdNode { t: None, n: cmd_topic, c: pub_count.clone() })
        .rate(100_u64.hz()).order(1).build();
    let _ = sched.run_for(Duration::from_millis(dur_ms));
...
fn xps_bidirectional_2_processes_with_schedulers() {
    ...
    let mut proc_a = spawn("xps_bidirectional_2_processes_with_schedulers", &topic, "bidir_a", dur, 100, 40);
    let mut proc_b = spawn("xps_bidirectional_2_processes_with_schedulers", &topic, "bidir_b", dur, 100, 50);
    ...
    println!("╠══════════════════════════════════════════════════════════════╣");
    println!("║  Process A: pub {:5} Imu,    recv {:5} CmdVel  corrupt={}  ║", a_pub, a_recv, a_corrupt);
    println!("║  Process B: pub {:5} CmdVel, recv {:5} Imu     corrupt={}  ║", b_pub, b_recv, b_corrupt);
    println!("╚══════════════════════════════════════════════════════════════╝");
