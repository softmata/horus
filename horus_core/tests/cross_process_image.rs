    ...
    while received_count < expected_count && Instant::now() < deadline {
        match topic.recv() {
            Some(img) => {
                ...
                // Validate pixel pattern — try all possible frame indices
                // to handle the dummy frame offset (publisher sends a dummy
                // before the numbered frames, which may shift the index).
                let data = img.data();
                let mut found_match = false;
                for candidate in 0..=(expected_count as u32 + 1) {
                    if validate_pattern(&img, candidate) {
                        valid_count += 1;
                        found_match = true;
                        break;
                    }
