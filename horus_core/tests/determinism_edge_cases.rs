...
impl Node for PubSubNode {
    ...
    fn publishers(&self) -> Vec<TopicMetadata> {
        self.pubs.iter().map(|t| TopicMetadata { topic_name: t.clone(), type_name: "T".into() }).collect()
    }
    fn subscribers(&self) -> Vec<TopicMetadata> {
        self.subs.iter().map(|t| TopicMetadata { topic_name: t.clone(), type_name: "T".into() }).collect()
    }
    fn tick(&mut self) {
...
fn node_without_rate_in_deterministic() {
    ...
    struct SimpleCounter { count: Arc<AtomicU64> }
    impl Node for SimpleCounter {
        fn name(&self) -> &str { "no_rate" }
    ...
    let mut scheduler = Scheduler::new()
        .deterministic(true)
        .tick_rate(100_u64.hz());
    scheduler.add(SimpleCounter { count: count.clone() })
...
fn mixed_rt_and_besteffort_deterministic() {
...

    {
        ...
        scheduler.add(OutputNode::new("rt_node", rt_out.clone()))
            .order(0).rate(100_u64.hz()).build().unwrap();
        scheduler.add(OutputNode::new("be_node", be_out.clone()))
    ...
    assert_eq!(be_out.lock().unwrap().len(), 10, "BestEffort should tick 10 times");
}
...
fn many_nodes_deterministic() {
    ...
    for i in 0..10 {
        ...
        let subs = if i > 0 { vec![format!("topic_{}", i - 1)] } else { vec![] };
        scheduler.add(PubSubNode {
            name: format!("node_{}", i),
            pubs,
            subs,
            log: log.clone(),
    ...
    for i in 0..10 {
        let pos = entries.iter().position(|n| *n == format!("node_{}", i)).unwrap();
        if i > 0 {
            let prev_pos = entries.iter().position(|n| *n == format!("node_{}", i - 1)).unwrap();
    ...
    for i in 10..13 {
        let pos = entries.iter().position(|n| *n == format!("consumer_{}", i)).unwrap();
...
fn run_n_ticks(n: usize) -> Vec<u64> {
    let out = Arc::new(Mutex::new(Vec::new()));
    {
        let mut scheduler = Scheduler::new()
            .deterministic(true)
            .tick_rate(100_u64.hz());
        scheduler.add(OutputNode::new("test", out.clone()))
            .order(0).rate(100_u64.hz()).build().unwrap();
        for _ in 0..n {
            scheduler.tick_once().unwrap();
        }
...
fn cycle_detection_returns_error() {
    ...
    assert!(result.is_ok(), "Cycle should be handled gracefully, not crash");
}
...
fn budget_remaining_decreases_during_work() {
    ...
    impl Node for BudgetChecker {
        fn name(&self) -> &str { "budget_checker" }
        fn tick(&mut self) {
            ...
            self.readings.lock().unwrap().push(before.as_secs_f64() - after.as_secs_f64());
        }
    }
    ...
    scheduler.add(BudgetChecker { readings: readings.clone() })
        .order(0)
        .rate(100_u64.hz())
        .budget(1_u64.ms())
        .build()
...
fn horus_dt_fixed_in_deterministic() {
    ...
    let mut scheduler = Scheduler::new()
        .deterministic(true)
        .tick_rate(200_u64.hz()); // 5ms period

    scheduler.add(DtChecker { dts: dts.clone() })
    ...
    for (i, &dt) in values.iter().enumerate() {
        assert_eq!(dt, 5_000_000, "Tick {} dt should be exactly 5ms, got {}ns", i, dt);
    }
}
