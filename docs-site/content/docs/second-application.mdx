...
- Verify with dashboard: `horus dashboard` -> Topics tab
...
### Issue 3: Dashboard Shows No Nodes

**Symptom:** Dashboard is empty

**Cause:** Application not running or dashboard started before app

**Fix:**
1. Start the application first: `horus run`
2. Then start dashboard in separate terminal: `horus dashboard`
3. Dashboard auto-discovers running nodes
...
3. **[node! Macro](/core-concepts/node-macro)** - Reduce boilerplate with macros
4. **[Message Types](/core-concepts/message-types)** - Use complex message types instead of primitives
...
For additional examples, see [Basic Examples](/basic-examples).
