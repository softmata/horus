...
horus dashboard               # Web-based monitoring and visualization
...
**Example:**
```rust
scheduler.add(keyboard_node, 0, Some(true));   // Runs first
scheduler.add(controller, 5, Some(true));      // Processes input
scheduler.add(motor_driver, 10, Some(true));   // Actuates
```
...
2. Common installation paths: `~/horus/HORUS`, `/horus`, `/opt/horus`, `/usr/local/horus`
...
- **[Goals & Vision](/goals)**: Understand what HORUS is trying to achieve
- **[API Reference](/api)**: Detailed API documentation
- **[Examples](/basic-examples)**: Complete example applications
