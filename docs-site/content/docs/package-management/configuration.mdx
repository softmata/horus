...
Shared session identifier for cross-process communication.

**Type**: String
**Required**: No
**Default**: Auto-generated UUID (unique per `horus run`)
**Use Case**: Multi-process applications that need to share topics

**When to Use:**
- Building multi-process apps (backend + GUI)
- Multiple related processes communicating via Hub
- Want isolation from other apps

**When NOT to Use:**
- Single-process applications
- Need system-wide topics (use `Hub::new_global()` instead)

**Examples:**

**Multi-process app (backend + GUI):**
```yaml
# backend/horus.yaml
name: robot_controller
version: "0.1.0"
session_id: "robot-control-session"

# gui/horus.yaml
name: robot_dashboard
version: "0.1.0"
session_id: "robot-control-session"  # Same session ID!
```

Both processes can now communicate:
```rust
// backend.rs
...
```
/dev/shm/horus/sessions/robot-control-session/topics/sensors
```

**See Also**: [Cross-Session Communication](/core-concepts-shared-memory#cross-session-communication)
...
license: "Apache-2.0"
session_id: "robot-system-v1"  # Shared with GUI
mode: release
language: rust
...
version: "1.0.0"
description: "Real-time robot dashboard and visualization"
author: "ACME Robotics"
license: "Apache-2.0"
session_id: "robot-system-v1"  # Shared with backend
mode: release
language: rust
...
```yaml
# Using session_id to enable backend-GUI communication
session_id: "robot-fleet-alpha"
...
- **[Cross-Session Communication](/core-concepts-shared-memory#cross-session-communication)** - Multi-process apps
