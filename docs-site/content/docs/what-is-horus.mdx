...
**Note:** HORUS supports both **typed messages** (Pose2D, CmdVel, etc.) and **generic messages** (dicts/JSON). Typed messages provide better performance and type safety. See [Message Types](/core-concepts/message-types) for details.
...
4. Dashboard monitors the system in real-time
...
3. [Basic Examples](/basic-examples) - Working examples
...
3. **[See Examples](/basic-examples)** - Learn from real projects

For command reference, see [CLI Reference](/development/cli-reference). For architecture details, see [Architecture](/architecture).
