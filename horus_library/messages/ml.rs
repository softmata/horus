...
//
// These are serde-based types for flexible ML workflows. For zero-copy IPC
...
/// ML trajectory point for imitation learning
///
/// Single observation-action pair for behavior cloning.
/// Named `MlTrajectoryPoint` to distinguish from `control::TrajectoryPoint`
/// (the Pod type for robot trajectory waypoints).
#[derive(Clone, Debug, Serialize, Deserialize)]
...
impl LogSummary for Predictions {
    fn log_summary(&self) -> String {
        format!(
...
impl LogSummary for InferenceMetrics {
    fn log_summary(&self) -> String {
        format!(
